...
2. **Issue #441 — fix the dead `macd_hist` input BEFORE the MAD PR-2 WIRING** — `pillars.py` checks `isinstance(macd, dict)` but `technical.macd_signal()` returns a float, so `macd_hist` is always NaN and the technical pillar runs 4-of-5 inputs. The #447 diagnostics (PR-1) deliberately left it untouched; the fix gates the WIRING step — MAD (`mad_scalefree`, scout #442 + diagnostics #447) must be wired against the FIXED baseline or its IC comparison is meaningless.
