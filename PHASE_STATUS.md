...
| Subagent inventory | **29** in 6 tiers (6 opus + 21 sonnet + 2 fable; all 29 `effort: ultracode`) — roster + routing matrix: `.claude/agents/README.md` (Tier 4 adds the cross-cutting `agent-output-verifier` "จับผิด" fact-checker + the `loop-engineer` work-loop architect; Tier 6 adds the two read-only **Fable 5** prose seats `narrative-copywriter` + `ux-microcopy-writer`) |
...
- **Nothing in flight** (re-verified 2026-07-02). Since the last bump
  (2026-06-27, #631/#632) three schema PRs merged: **#665** (Phase 9.1
  broad-universe probe, 0.10.42, 2026-06-29) · **#652** (`restatement_history`
  weight demotion, 0.10.43, 2026-06-29) · **#684** (ADV-floor decoupling +
  shadow floor-sweep, 0.10.44, 2026-07-02) — see §Chronological history
  2026-07-02. ~29 additional no-schema-bump PRs also merged in this window
  (orchestrator refactors #259-R1..R7b, Phase 9.3 CI/runtime PRs #667/#668/
  #676/#681, frontend/agent-roster PRs) — not individually enumerated here;
  see `git log` / PHASE_STATUS_INFLIGHT.md. The legendary-fund 6-proposal
  program (`claude/fund-performance-rankings-f8x4o1`, PRs
  #604/#607/#615/#617/#624/#628) merged complete 2026-06-26 (schema
  0.10.33→0.10.40). Proposal B (PEG/GARP) was REJECTED, not deferred
  (φ(PEG,P/E)=0.849 on sp1500 > 0.5 gate — double-loads P/E). See
  §Chronological history 2026-06-26 for the full per-PR record.
...
- **Tags are now CHRONOLOGICAL, decoupled from phase numbers.** `v2.0.0-phase8`
  shipped 2026-06-23; **the next tag is v2.1**, then v2.2 … No more phase-pinned tags
  after v2.0 (the `-phase8` suffix on the latest tag is the last one; that pinning is
...
- **#544** — `low_liquidity` annotate → veto promotion. Firing data now EXISTS —
  ...
  (post-cron audit, 2026-06-22). methodology-scientist RATIFY-WITH-CONDITIONS
  2026-06-23 (DOCS-ONLY, not yet wired). **Post-v2.0 deferred — KEEP-ANNOTATE through
  v2.0; promote at/after the Q3 2026-08-19 audit.**
...
- **Latest tag: `v2.0.0-phase8`** (SHIPPED via #577, 2026-06-22) — the S&P 1500
  universe cutover release (the 502→~1504 production expansion + Slice 8 #548/#564 +
  display tiles #549/#578 + warehouse #539/#547 + data-integrity #555/#571/#573). Prior
  tag: `v1.4.0-phase4.6` (2026-05-27, commit `a820caee`).
...
## 2026-07-02 — `broad_investable_us` universe-floor scoping PR-1: ADV-floor decoupling + shadow floor-sweep (#684)

Schema `0.10.43-phase9pilot` → `0.10.44-phase9pilot` (squash `684e36ec`). SHADOW-only
(Rule 18) — no floor flipped; rankings/scores/flags BYTE-IDENTICAL on every path;
defense layer UNCHANGED at 38.

- New `BROAD_UNIVERSE_ADV_FLOOR_USD = 5_000_000.0` constant (== `ADV_FLOOR_USD` at
  launch) decouples a latent shared-constant defect between the RANKED-path
  investability screen and the sp1500 `low_liquidity` annotate floor.
- New pure `sweep_broad_universe_floors` + `_adv_survivors_at_floors` in
  `compute/ingest/broad_universe.py` — monotone-nested survivor counts at
  $5M/$10M/$15M.
- 6 new write-only `Metadata` fields: `broad_universe_survivors_at_5m_count` /
  `_at_10m_count` / `_at_15m_count` / `broad_universe_adr_suspect_count`
  (structurally `None` today) / `broad_universe_survivor_fundamentals_coverage_pct`
  (structurally `None` today) / `broad_universe_sector_unknown_pct` (populates
  ~100% today).
- Design provenance: financial-engineer → literature-searcher (Fama-French 2008
  *JF* 63(4) + Hou-Xue-Zhang 2020 *RFS* 33(5)) → methodology-scientist
  RATIFY-WITH-CONDITIONS (dollar level deferred to PR-2's empirical sweep +
  acceptance bands C1-C6 + owner sign-off).
- Reviewed: quantrank-reviewer + schema-sentinel + defense-layer-auditor +
  agent-output-verifier (zero regression). +33 offline tests.
- Follow-ups open: PR-2 floor flip (gated); promote FF2008/HXZ2020 into
  `docs/METHODOLOGY.md` §universe-construction (not yet done).

## 2026-06-29 — Phase 9.1 Broad Investable US probe (#665) + `restatement_history` weight demotion (#652)

Two schema PRs merged same day, in this order:

- **#665** (squash `ca3b1f06`) — schema `0.10.41`→`0.10.42-phase9pilot`. 6 new
  write-only `Metadata.broad_universe_*` fields; wired unconditionally into the
  weekday cron the same day by **#667** ("Phase 9.1b"), NO further schema bump.
  Rankings byte-identical.
- **#652** (squash `135bf9f7`) — schema `0.10.42`→`0.10.43-phase9pilot`.
  `RESTATEMENT_HISTORY_WEIGHT` 5.0→0.0, `RESTATEMENT_HIGH_CONFIDENCE_WEIGHT`
  3.0→8.0 (preserves irregularity total). RANK-NEUTRAL; defense UNCHANGED at 38.

Both PRs' full detail already lives in PHASE_STATUS_INFLIGHT.md and CLAUDE.md
§Gotchas — this entry closes the Chronological-history / Current-state gap only.
