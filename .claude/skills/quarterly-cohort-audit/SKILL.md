...
description: Quarterly walk of the defense layer (active vetoes + annotates + method-applicability + informational flags) against academic priors on the latest compute output. Surfaces over-/under-firing flags vs expected bands, undocumented flags missing from the headline count, and delta vs the prior quarter; lands as an issue comment. TRIGGER: the scheduled quarterly date (next 2026-08-19), methodology-scientist Mode C, or "cohort audit" / "audit the flags against literature".
---
