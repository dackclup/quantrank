...
- **`dividend_yield_pct` is PERCENT (×100), `payout_ratio` is a 0-1 fraction (#512)** — yfinance `.info["dividendYield"]` is a fraction (0.0123 = 1.23%); `pays_dividend = True iff dividend_yield_pct > 0` (tri-state, `None` when absent). `fetch_yfinance_dividend` is a pure cache-read off the warm `yfinance_info` cache (zero new round-trips). The `HeroAttributeTiles` "Dividend" tile is gated on ≥ 1 cron confirming `Metadata.dividend_coverage_pct` populates (observability-before-wiring, `exchange_coverage_pct` precedent).
