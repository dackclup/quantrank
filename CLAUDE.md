...
- **`/sectors` + `/movers` + the home/ranking dashboards derive stats from `rankings.json`/`metadata.json` at BUILD TIME (Server Components) — weekly static export, so values are "as of" the last cron, NOT live; never `import lib/data.ts` (or any `fs` module) into a `'use client'` component (resolve on the server, pass the node in as a prop/child). The Seeking-Alpha-style top `MarketStatsBar` strip + `lib/market-stats.ts` + the `AppShell` `topBar` slot were REMOVED 2026-06-04 (the build-time server-component rule lives on via `/sectors` + `/movers`)**
