...
- **Price-chart resolution is PER-PERIOD — 5Y aggregates to monthly, the
  ...
  **daily OHLCV, ~1254 points over 5y** (`compute/config.PRICES_PERIOD="5y"`,
  ...
  per calendar month = the close of that month's LAST trading day, ~60 points
  over 5y, the standard long-range convention); **1M / 6M / YTD / 1Y → daily**,
  ...
  no longer even-stride-downsamples daily points — it month-end-aggregates.
  `aggregateMonthly` assumes the series is ascending by `date` (it is, per
  `write_stock_history`) and keeps the most-recent partial month's latest close
  as the final point (e.g. a mid-month "today"). **1D (1-min) / 5D (15-min) are
