...
| `.claude/agents/` | 29 project subagents in 6 tiers (Core · Lifecycle · Specialized · Operations incl. the cross-cutting `agent-output-verifier` "จับผิด" seat + the `loop-engineer` work-loop architect · write-capable Builders: `compute-builder` owns `compute/**`, `frontend-builder` owns `frontend/**` · Creative: the two **Fable 5** read-only prose seats `narrative-copywriter` (long-form) + `ux-microcopy-writer` (in-product strings)). Routing matrix + 9 coordination flows: [`.claude/agents/README.md`](.claude/agents/README.md); agent-team recipes: [`.claude/agents/TEAMS.md`](.claude/agents/TEAMS.md). |
...
The main session is the **orchestrator / tech lead** of the 29-agent
...
`description:` fields of all 29 agents before defaulting to inline.
...
| Task IS reader-facing long-form PROSE (README pitch / release-notes or changelog narrative / announcement / blog / social / plain-English methodology explainer) — "make the pitch compelling" / "draft the release story" / "เขียนให้อ่านลื่น" / "เกริ่นนำ" | `narrative-copywriter` (**fable**, read-only) — drafts voice & tone; `[VERIFY]` for any unsourced figure; accuracy gate stays `docs-reviewer` / `agent-output-verifier`. Route here ONLY when the deliverable is the prose itself — a factual/code task never lands on a fable seat |
| Task IS in-product MICROCOPY (empty-state / "no matches" copy · tooltip · badge/chip/button label · loading/error/toast string · `aria-label`) — "write the empty-state copy" / "microcopy for X" / "reword this label" / "make this string friendlier" / "ข้อความปุ่ม" | `ux-microcopy-writer` (**fable**, read-only) — drafts the STRING; `frontend-builder` wires it, `frontend-design-reviewer` tone-checks. Not code, not design tokens, not long-form prose |
...
- **`cache-v12-fast` is the current fast-bundle key (Phase 9.3 runtime pre-req, 2026-06-29)** — `cache-v11-fast`→`cache-v12-fast` cold-seed bump across all 4 workflows (compute-rankings · precache-edgar · pre-merge-prod-sim · backfill-portfolio) so the new `compute/cache/broad_universe-v1.parquet` path (Phase 9.1 Broad Investable US probe, 7-day TTL) is included in the fast bundle going forward. A warm v11 exact-key save would silently omit the new path — same frozen-fast-cache mechanism as v10→v11 (sp1500 Slice 5 #520) and v9→v10 (sp900 Phase B #492). ~~Also adds `broad_investable_us` to `workflow_dispatch` options in compute-rankings + precache-edgar~~; backfill-portfolio save key is `cache-v12-bf-` (subset-safe, avoids poisoning the quarter with an impoverished bundle). ~~CI timeout bumps: weekday cron 270→420 min (sp1500 cold + broad-universe cold-fetch headroom), Saturday precache 270→540 min (full broad-universe cold worst-case ~360 min).~~ 8-K jitter window widened 72h→96h (`EDGAR_8K_CACHE_TTL_JITTER_SECONDS` in `compute/config.py`): at ~3,545 broad-universe names the Sat 08:00→Mon 22:00 UTC gap is ~62h; W=72h risked re-bunching the cold-burst expiry; W=96h spreads across Sat 08:00→Wed 08:00 (4-day window). Cron universe default UNCHANGED at sp1500; schema + scoring + defense UNCHANGED. Prior: `cache-v11-fast` (#520, S&P 1500 Slice 5 — sp600/sp1500 parquet cache paths); slow-text bundle key (`cache-v5-text-`) unchanged. **CORRECTED 2026-07-02 (Phase 9.3 gate-C1 fix, see the entry two below)**: the struck-through claims above were WRONG / superseded — `broad_investable_us` was REMOVED from precache-edgar.yml's dispatch options (it never fit that workflow's per-job budget), and the "~360 min cold worst-case" broad-universe projection was WRONG (observed ~480-510 min, past even the 540-min ceiling). Both `compute-rankings.yml` (420→360) and `precache-edgar.yml` (540→360) now cap at 360 — GitHub Actions' HARD, unmovable 6-hour per-job ceiling; a value above it was always dead configuration. Broad-universe COLD precaching moved to the dedicated 4-job `precache-broad-universe.yml` split.
...
- **`broad_universe_*` (6 Metadata fields, Phase 9.1, schema 0.10.42) are WRITE-ONLY / SHADOW on the sp1500 cron — HARD NAMING CONSTRAINT + Rule 18** — `compute/ingest/broad_universe.py` fetches the "Broad Investable US" candidate pool from `SEC_COMPANY_TICKERS_URL` (config constant), applies exchange/name/ticker-format exclusions, then `screen_broad_universe_investability` applies price ≥ $5 / ADV ≥ $5M using the Step-1 `prices_by_ticker` dict already in memory (no extra yfinance calls). 6 new `Metadata` fields: `broad_universe_raw_count` / `broad_universe_candidate_count` / `broad_universe_screened_count` / `broad_universe_price_fail_pct` / `broad_universe_adv_fail_pct` / `broad_universe_coverage_pct`. **Runs on the weekday cron** (Phase 9.1b, wired in `run_weekly_compute` immediately after Step 1 prices, reusing the SP1500 ~1502-name `prices_by_ticker` dict — no extra yfinance round-trips; `broad_universe_coverage_pct` expected ~40–45% on 9.1 Probe slice, the SP1500∩Broad overlap / Broad total). **Pre-merge sim sets `QR_SKIP_BROAD_UNIVERSE=1`** to avoid the live SEC company_tickers.json fetch (bundled-parquet path is non-deterministic on CI runners; Dependabot PRs have no `EDGAR_USER_AGENT`). HARD NAMING CONSTRAINT (legal/trademark, 2026-06-29): NEVER "Russell 3000" / "Russell-3000-class" / "equivalent to Russell 3000" in ANY field name, label, comment, log line, or user-visible string in the repo. HARD RULE 18 CONSTRAINT (sp1500-cron probe path only — see the Phase 9.3 entry below for the RANKED-path exception): all six fields MUST NEVER be read by scoring, composite, pillar computation, veto/flag logic, fair-price, or `select_picks` — write-only, observability-only; rankings/scores/flags BYTE-IDENTICAL; defense layer UNCHANGED at 38. Escape hatch: `QR_SKIP_BROAD_UNIVERSE=1`. Disk cache: `compute/cache/broad_universe-v1.parquet`, 7-day TTL. `EDGAR_USER_AGENT` is a HARD ERROR (not graceful degradation) on absence. Tenacity: `stop_after_attempt(3)` / `wait_exponential(min=2, max=20)`. Phase 9.3 (merged — see next entry) extends the price-fetch to the full candidate pool on the DISPATCH-ONLY ranked path and pushes `broad_universe_coverage_pct` toward ~95%+ THERE ONLY; on the sp1500-cron probe path the ~40–45% coverage semantics are unchanged.
- **`QR_UNIVERSE=broad_investable_us` is a RANKED, DISPATCH-ONLY universe path (Phase 9.3) — the scheduled cron default stays `sp1500`, UNCHANGED** — `compute/main.py`'s universe-selector seam gains a fourth `elif config.QR_UNIVERSE == "broad_investable_us":` branch (after `sp1500`, before the `else` sp500 default) that loads the ~6,883-name Broad Investable US candidate pool via `candidates_to_universe_frame(fetch_broad_universe_candidates())` (new function in `compute/ingest/broad_universe.py`) and makes it the `universe` frame fed to Step 1. Step 1 (`fetch_all_prices`) therefore fetches prices for the FULL ~6,883-name candidate pool on this path — NOT a restricted sp1500 dict. Immediately after Step 1 (and after the unconditional Phase-9.1 probe block, which now ALSO benefits: on this path its `broad_universe_screened_count`/`_coverage_pct` describe the TRUE screen result, not a lower bound), a SEPARATE gated block (`if config.QR_UNIVERSE == "broad_investable_us":`) calls `select_broad_universe_survivors(candidates=universe, prices_by_ticker=prices_by_ticker)` (a NEW function, distinct from the Rule-18 diagnostic `screen_broad_universe_investability` — returns a ticker SET, not counts) and reduces `df` / `rows` / `prices_by_ticker` / `adv_by_ticker` to the ~3,545 investability-screen survivors BEFORE Step 2 (fundamentals) — so EDGAR load stays bounded at roughly sp1500 scale, never all ~6,883. **P1-G3 methodology gate** (methodology-scientist ratified): non-survivors are REMOVED from the peer set entirely — they do NOT get a `low_liquidity` annotate (that annotate is for names that ARE ranked but trade thinly; a name that never enters the scored frame has nothing to annotate). **Failure mode is ABORT, not fall-through**: unlike most graceful-degradation blocks in `run_weekly_compute`, a raised exception from `select_broad_universe_survivors` returns 0 (mirrors the `assert_sec_api_usable` / `MIN_VALID_TICKERS` abort idiom) rather than silently scoring the full unscreened ~6,883-name frame — falling through would double the fundamentals-fetch volume this path explicitly bounds and risk a CI timeout. `Metadata.universe` emits `"BROAD_INVESTABLE_US"` (a new fourth branch in the label ternary, alongside the existing `"SP1500"`/`"SP900"`/`config.UNIVERSE` cases). `derive_index_memberships` (`compute/ingest/universe.py`) renames its internal `_SP600_COHORTS` guard set to `_NO_RUSSELL1000_PROXY_COHORTS = {"sp600", "broad"}` — a `cohort="broad"` ticker (set by `candidates_to_universe_frame`) never gets the `russell1000` proxy tag even with a known positive market cap, because unlike the S&P 900 there is no structural "this cohort's floor sits above the Russell 1000 floor" argument for the broad, unfiltered-at-the-top candidate pool. **P1-G4 RE-NORMALIZATION DISCLOSURE (methodology-REQUIRED)**: broadening the scored universe from S&P 1500 (~1504 names) to Broad Investable US (~3,545 names) RE-BASES every cross-sectional percentile and sector median the 8-pillar composite uses — a score computed on this path is NOT comparable to the same ticker's sp1500-cron score. GICS sector is unavailable from the SEC company-tickers source, so every candidate gets `sector="Unknown"` (mirrors the EXACT precedent already in the S&P 400/600 Wikipedia-table loader's graceful-degradation path) — sector-exclusion checks are exact-string comparisons against `"Financials"`/`"Utilities"` so `"Unknown"` never trips them, and `get_cost_of_equity` falls back to the flat 10% default. **No schema change** — the same 6 `broad_universe_*` Metadata fields (Phase 9.1) suffice; distinguish "probe-only, still an sp1500 cron" (lower-bound semantics) from "the ranked broad-universe run" via `Metadata.universe == "BROAD_INVESTABLE_US"` (true-count semantics). PANDAS 3.0 GOTCHA caught while building `candidates_to_universe_frame`: assigning a Python list / `pd.array(..., dtype=object)` containing a mix of strings and `None` into a DataFrame column silently coerces every `None` back into a float `NaN` under pandas 3.0's automatic string-dtype inference — a `NaN` in the `cik` column is a live corruption risk (`bool(float("nan"))` is `True`, so `r.get("cik") or ""` downstream keeps the NaN instead of falling through to `""`, and `str(nan_value)` hands `fetch_fundamentals` the literal string `"nan"` as a CIK). Fix: wrap the values in `pd.Series(..., dtype=object, index=out.index)` BEFORE assigning to the column — constructing the Series with an explicit dtype up front bypasses the content-based inference a subsequent bare-list assignment triggers. AI-pick basket / backtest stay SP500-only in this PR (broadening those is Phase 9.4 scope); the frontend disclaimer for a broad-universe run is also Phase 9.4 scope. HARD NAMING unchanged: "Broad Investable US" everywhere, never "Russell 3000". Draft PR — offline tests only (`tests/test_ingest/test_broad_universe_ranked.py`, 40 tests incl. 6 `TestDefaultPathUnchangedRegressionGuard` tests pinning verbatim sp900/sp1500/sp500-else source snippets); no live sp1500/sp900/sp500 cron has exercised this branch yet (dispatch-only, untested on real data until a manual `workflow_dispatch` run).
- **Phase 9.3 gate-C1 fix — broad-universe COLD precaching needs a 4-job split; a single job hits GitHub Actions' 6h hard cap (2026-07-02, DRAFT)** — a single-job cold run of the `QR_UNIVERSE=broad_investable_us` RANKED path (~6,883-name candidate pool → Step-1 prices → investability screen → ~3,545 survivors → fundamentals/history/form4/tier2 fetched only for survivors) was CANCELLED at GitHub Actions' HARD, unmovable 6-hour (360-minute) per-job ceiling with NO cache saved — observed ~480-510 min cold, past even the (wrong) 540-min `precache-edgar.yml` ceiling that existed at the time. **`timeout-minutes:` above 360 is ALWAYS dead configuration** — GHA silently clamps enforcement to 360 regardless of the declared value; both `compute-rankings.yml` (was 420) and `precache-edgar.yml` (was 540) are corrected to 360 (see the `cache-v12-fast` entry above, now struck through where superseded). **Fix**: new `.github/workflows/precache-broad-universe.yml`, `workflow_dispatch`-ONLY (no `schedule:` — an occasional cold-warming pass, not a recurring cron), 4 SEQUENTIAL jobs via a linear `needs:` chain, each individually ≤ 360 min: job 1 `price-screen` (120 min — fetches the full ~6,883-candidate pool's prices, runs `select_broad_universe_survivors`, uploads the survivor ticker frame `ticker/cik/name/sector/cohort` as a build artifact via `actions/upload-artifact`) → job 2 `fundamentals-history` (150 min — downloads the artifact, calls `fetch_all_fundamentals` + `fetch_all_history`) → job 3 `form4` (300 min — historically the single most expensive EDGAR loop at scale) → job 4 `tier2` (330 min — the largest budget; 10-K text + 8-K events). Each of the 4 thin `scripts/precache_broad_stage_*.py` scripts calls the SAME orchestrator helpers `compute.main.run_weekly_compute` calls for the ranked path, so on-disk cache writes are byte-identical to what that path produces. **Cache-key strategy (avoids the exact-key-skip-save trap — same bug class as #471 frozen-fast-cache)**: every job RESTORES the `-broad-`-namespaced quarter/os prefix first (inherits an earlier job's/run's warm state), falling back to the CANONICAL `cache-v12-fast-<quarter>-` / `cache-v5-text-<os>-` prefix (inherits sp1500 warmth — ~1,500 of the ~3,545 survivors overlap). Every job SAVES under a UNIQUE per-job key (`cache-v12-fast-<quarter>-broad-${{ github.run_id }}-job2` / `cache-v5-text-<os>-broad-${{ github.run_id }}-job3`, etc.) — NEVER the bare canonical key, so the save is never skipped by `actions/cache`'s exact-key immutability AND the sp1500 canonical bundle can never be overwritten mid-precache with a partial snapshot. Because a broad-namespaced key STARTS WITH the canonical restore-key prefix, a LATER `compute-rankings.yml universe=broad_investable_us` dispatch restoring via that prefix naturally inherits this workflow's warmed data too — no extra cron-side wiring needed. Joins `concurrency: group: edgar-cache-writers` (same group as the other two cache writers). **`compute/orchestrator/history.py` (new)** — a prerequisite extraction: Step 3 (annual-history fetch) was the one EDGAR-bound loop NOT already independently callable (unlike prices/fundamentals/form4/tier2, extracted in PRs #259-R2..R5) — needed so `scripts/precache_broad_stage_fundamentals.py` can call it outside `run_weekly_compute`. Pure code move mirroring the `fundamentals.py` (R3) pattern exactly (`fetch_all_history(df, *, max_workers, timeout) -> dict[str, pd.DataFrame]`); `compute.main` no longer defines `_history_one` or imports `fetch_fundamentals_history` directly. NOT itself part of the `#259` R-numbered series (R7a/R7b are reserved for the unrelated per-ticker Step-8 loop extraction) — a standalone prerequisite for this fix. `precache-edgar.yml`'s `universe` dispatch input DROPS `broad_investable_us` (moved to the dedicated workflow); `compute-rankings.yml` KEEPS it (the warm RANKED dispatch still lives there). No schema change; default weekday sp1500 cron path is BYTE-IDENTICAL (history-extraction is a pure code move, verified by dedicated regression tests incl. `compute.main` no longer defining `_history_one`). Full design + verification ladder: `PHASE_STATUS_INFLIGHT.md`.
...
**In flight**: nothing currently in flight (re-verified 2026-07-02 — everything the prior revision tracked below has since merged). **`restatement_history` weight demotion (issue #16)** MERGED as **#652** (squash `135bf9f7`, 2026-06-29) — DEMOTE the bare annotate's manipulation-index weight 5.0 → 0.0 (Q3 cohort audit: over-fires 17.82% on SP1500 vs HLM 2008 1-3%; fraud-class content already carried by `restatement_high_confidence`, raised 3.0 → 8.0 to preserve the irregularity total) + new shadow `Metadata.restatement_history_weight_demote_delta_count`; schema `0.10.42-phase9pilot` → **`0.10.43-phase9pilot`**; RANK-NEUTRAL, live rankings BYTE-IDENTICAL; defense UNCHANGED at 38 (DEMOTE not DEPRECATE); full entry in [`PHASE_STATUS_INFLIGHT.md`](PHASE_STATUS_INFLIGHT.md). **Phase 9.3 runtime pre-req MERGED as #668** (2026-06-29; ci: Phase 9.3 GHA budget + cache-v12-fast cold-seed — ~~timeout 270→420 min cron / 540 min precache~~; cache key v11→v12; `broad_universe-v1.parquet` in fast-bundle path; ~~`broad_investable_us` dispatch option~~; 8-K jitter 72h→96h; NO schema bump, NO scoring change, cron universe default UNCHANGED at sp1500) — **the struck-through timeout/dispatch-option claims are SUPERSEDED by the gate-C1 fix below** (420/540 both exceeded GitHub Actions' hard 6h/360-min per-job cap and were dead configuration; `broad_investable_us` moved off `precache-edgar.yml`'s dispatch options entirely). **Phase 9.3 RANKED-path wiring** (branch `claude/phase-9-3-broad-universe-ranked`, **MERGED as #676**) — wires `QR_UNIVERSE=broad_investable_us` as a scored/ranked universe (new `elif` branch in `compute/main.py`'s universe-selector seam; new `candidates_to_universe_frame` + `select_broad_universe_survivors` in `compute/ingest/broad_universe.py`; screen-then-reduce-before-fundamentals sequencing bounds EDGAR load to ~3,545 survivors, never all ~6,883 candidates; P1-G3 non-survivors removed from peer set entirely, no `low_liquidity` annotate; P1-G4 re-normalization caveat documented; `derive_index_memberships` gains a `"broad"` russell1000-suppression cohort). DISPATCH-ONLY — scheduled cron default stays sp1500, UNCHANGED; AI-pick/backtest stay SP500-only (Phase 9.4 scope); NO schema bump (reuses the existing 6 Phase-9.1 `broad_universe_*` fields). A real pandas-3.0 `None`→`NaN` CIK-corruption bug was caught and fixed mid-build (see CLAUDE.md §Gotchas + `PHASE_STATUS_INFLIGHT.md` for the repro). **Phase 9.3 gate-C1 fix MERGED as #681** (branch `claude/phase-9-3-precache-broad-split`, 2026-07-02) — a single-job cold `universe: broad_investable_us` dispatch was CANCELLED at GHA's 6h/360-min hard cap with no cache saved (~480-510 min observed cold); fix is new `.github/workflows/precache-broad-universe.yml` (`workflow_dispatch`-only, 4 SEQUENTIAL jobs each ≤ 360 min: price-screen 120 → fundamentals-history 150 → form4 300 → tier2 330, broad-namespaced per-job-unique cache SAVE keys with canonical-prefix restore fallback) + 4 new thin `scripts/precache_broad_stage_*.py` + a prerequisite extraction `compute/orchestrator/history.py` (Step 3 annual-history loop, mirroring the #259-R2..R5 pattern — NOT itself part of that R-numbered series) + `compute-rankings.yml`/`precache-edgar.yml` timeout corrections (420/540 → 360) + `precache-edgar.yml` drops the `broad_investable_us` dispatch option. NO schema bump; default sp1500 cron path BYTE-IDENTICAL (history-extraction is a pure code move). Full design + verification ladder: [`PHASE_STATUS_INFLIGHT.md`](PHASE_STATUS_INFLIGHT.md); gotcha detail: CLAUDE.md §Gotchas. Full entry in [`PHASE_STATUS_INFLIGHT.md`](PHASE_STATUS_INFLIGHT.md). Merged 2026-06-27: **#631** (feat(compute): Option-B dividend pool-and-redeploy SHADOW NAV, issue #620 — `nav.adaptive_div_pooled` artifact series + 2 canary `Metadata` fields; schema `0.10.40`→**`0.10.41-phase8pilot`**; SHADOW/obs-first, live rankings/NAV BYTE-IDENTICAL; backfill-verify Option B −1.03% net vs Option A, coverage 80.6%; headline flip = future PR + owner sign-off; defense UNCHANGED at 36 — squash `858bec01`) · **#632** (feat(frontend): drop per-row TWR `% price` + `since` sub-lines from Current-picks return cell — keep only the headline MWR "Your return"; frontend-only, NO schema bump; rankings byte-identical — squash `d772f66d`). The legendary-fund 6-proposal deep-research program (`claude/fund-performance-rankings-f8x4o1`, PRs #604/#607/#615/#617/#624/#628) merged complete 2026-06-26 (schema chain 0.10.33→0.10.40); **Proposal B (PEG/GARP) was REJECTED** (φ(PEG,P/E)=0.849 > 0.5 orthogonality gate — double-loads P/E; no issue opened). #620 dividend headline-flip (Option A→B) + `low_liquidity` veto promotion (#544, KEEP-ANNOTATE for v2.0) + Bonferroni provisional-threshold re-derivation (#654, split out of the closed #542/#564) deferred to the Q3 2026-08-19 cohort audit / future owner-signed PR. Merged
