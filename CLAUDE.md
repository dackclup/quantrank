...
| Before ACTING on a high-stakes agent claim (release GO / destructive command / Mark-Ready / "Top-5 rotated" / "coverage 99%") OR two agent reports disagree OR "จับผิด" / "fact-check this report" / "verify the agent's claims" / "ข้อมูลที่ ai พ่นมาถูกไหม" | `agent-output-verifier` (opus) — re-derives each checkable claim from ground truth; per-claim CONFIRMED/REFUTED/STALE/UNSUPPORTED/UNVERIFIABLE; routes the fix back to the owning agent. NOT per-report (cost) — fires when acting on a false claim is expensive |
