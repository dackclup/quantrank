...
| **`0.10.31-phase8pilot`** (#565 squash, merged 2026-06-22; layered on #564's `0.10.30-phase8pilot`) | Phase 8 / roadmap item 5 / 7b — Security-type signal | **Additive PATCH — observability-first (Rule 18)**: new `StockDetail.security_type: str \| None` (yfinance `fast_info.quote_type` → `_QUOTE_TYPE_LABEL` map: `EQUITY`→"Common stock", `ETF`→"ETF", …; unknown codes pass through) + `Metadata.security_type_coverage_pct: float \| None` coverage canary. Pure cache-read off the warm `yfinance_info` cache (zero new round-trips); `_yf_fast_exchange` widened to a 2-tuple `(exchange_code, quote_type)`. Descriptive metadata, NOT a scoring/veto input. ADR override deferred `TODO(#541 PR-1b)`. UI "Type" tile wired separately (#578). **Rankings byte-identical; defense layer UNCHANGED at 36.** |
| **`0.10.30-phase8pilot`** (#564 squash `62dbf4f8`, merged 2026-06-22; layered on #527's `0.10.29-phase8pilot`) | Phase 8 / S&P 1500 Slice 8 / Slice 3 — Bonferroni shadow | **Additive PATCH — observability-first (Rule 18)**: 3 new `Metadata` fields (all `\| None`): `bonferroni_shadow_flip_count` / `bonferroni_shadow_live_fire_count` / `bonferroni_shadow_provisional_fire_count`. New `compute/scoring/bonferroni_shadow.py` (Harvey-Liu-Zhu 2016 FWER α/n over the ~1500-name cross-section). SHADOW / OBSERVABILITY-ONLY — live scores/flags byte-identical; **defense layer UNCHANGED at 36.** methodology-scientist RATIFY-SHADOW pending first sp1500 cron firing-rate. |
