...
preserves it).
Schema is currently **`0.10.26-phase8pilot`** on `main` (#501 squash `72ee8667d`, merged 2026-06-19 — cross-source share-count-corruption SHADOW observability PR-1; 4 new `Metadata.cross_source_corruption_*` fields; MUTATES NOTHING, defense UNCHANGED at 35); prior **`0.10.25-phase8pilot`** (#499 squash `816cda0ea`, merged 2026-06-18 — `post_split_share_lag` HYBRID defense + folded leg-3 override, new `compute/ingest/splits.py`, `RawMetrics.shares_outstanding_pre_split_raw` + 3 `Metadata.*` counters, defense 34→35); prior **`0.10.24-phase8pilot`** (#496/PR-A merged 2026-06-18). **In flight:** _Nothing currently in flight._ See CLAUDE.md §Phase status + PHASE_STATUS_INFLIGHT.md.
