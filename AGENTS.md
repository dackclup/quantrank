...
.claude/agents/                   # 24 subagents (5 fable / 19 sonnet; 22 at `effort: max`, 2 at `high`: schema-sentinel + vercel-preview-auditor) — Tier 1 Core 5 (incl. stock-detail-auditor for per-stock JSON correctness) + Tier 2 Lifecycle 6 (incl. vercel-preview-auditor + expert-user-explorer for interactive end-to-end app usage) + Tier 3 Specialized 8 (incl. literature-searcher + financial-engineer for generative quant design + data-pipeline-engineer + data-analyst for data-layer health + analytics) + Tier 4 Operations 3 (incl. ci-triage-engineer) + Tier 5 Builders 2 (write-capable compute-builder + frontend-builder for agent-team Feature Squads, see TEAMS.md); Claude Code only — Copilot / Cursor / Devin do not auto-route to these
...
The 24 subagents under `.claude/agents/` follow the **gate-moment
...
on the sonnet pool (19 of 24 agents) should walk every relevant
