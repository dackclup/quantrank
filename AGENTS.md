...
│   └── universe.py               # S&P 500 / 400 / 600 + combined 900 / 1500 constituents (QR_UNIVERSE=sp500|sp900|sp1500)
...
preserves it).
**In flight**: **#616 simulate runner-ceiling** — pin the `pre-merge-prod-sim` `QR_UNIVERSE` to **sp500** (one-line) while the cron still ranks sp1500: a cold sp1500 sim (~3.6h) trips the ~4h hosted-runner ceiling; sp500 cold (~43min) still fetches live (real diff on the sp500 ∩ committed-main intersection) but stays under it. The skip-live-fetch fix (Option 4a, `QR_SIM_NO_LIVE_FETCH`) was REVERTED (full cold miss → 0 tickers priced < `MIN_VALID_TICKERS` → compute abort, no output). CI/test/docs only; rankings byte-identical; defense UNCHANGED at 38; guarded by `test_sim_pins_sp500_universe_per_616_exception`. See PHASE_STATUS_INFLIGHT.md. Phase 9.1 Broad Investable US universe coverage probe + Phase 9.1b cron-wiring MERGED (#667, 2026-06-30) — the probe now runs unconditionally on the weekday cron, schema `0.10.42-phase9pilot`. `restatement_history` manipulation-index weight demotion (issue #16) MERGED as **#652** (squash `135bf9f7`, 2026-06-29 — schema `0.10.42-phase9pilot`→`0.10.43-phase9pilot`; `RESTATEMENT_HISTORY_WEIGHT` 5.0→0.0 + `RESTATEMENT_HIGH_CONFIDENCE_WEIGHT` 3.0→8.0 preserving the irregularity total; RANK-NEUTRAL, rankings byte-identical; defense UNCHANGED at 38). `broad_investable_us` ADV-floor decoupling + shadow floor-sweep MERGED as **#684** (squash `684e36ec`, 2026-07-02 — schema `0.10.43-phase9pilot`→`0.10.44-phase9pilot`; see Current schema below for full detail). **Phase 9.3 RANKED-path wiring** (branch `claude/phase-9-3-broad-universe-ranked`, **MERGED as #676**) — wires `QR_UNIVERSE=broad_investable_us` as an actual SCORED/RANKED universe (DISPATCH-ONLY; the scheduled cron default stays sp1500, UNCHANGED). New `elif` branch in `compute/main.py`'s universe-selector seam + `candidates_to_universe_frame` / `select_broad_universe_survivors` in `compute/ingest/broad_universe.py`; screen-then-reduce-before-fundamentals sequencing bounds EDGAR load to the ~3,545 investability-screen survivors, never all ~6,883 candidates (P1-G3 methodology gate: non-survivors removed from the peer set entirely, no `low_liquidity` annotate). P1-G4 re-normalization caveat documented (broad-universe scores are NOT comparable to sp1500-cron scores). `derive_index_memberships` extends its russell1000-proxy-suppression set to cover the new `"broad"` cohort. NO schema bump (reuses the existing 6 `broad_universe_*` fields); AI-pick basket + backtest stay SP500-only (Phase 9.4 scope). HARD NAMING CONSTRAINT: "Broad Investable US" everywhere — NEVER "Russell 3000" / "Russell-3000-class" / "equivalent to Russell 3000"; HARD RULE 18: the 6 `broad_universe_*` Metadata fields MUST NEVER be read by scoring, composite, pillar, veto/flag, fair-price, or `select_picks` on the sp1500-cron probe path (the Phase 9.3 ranked path is the documented exception where these SAME fields describe the true scored-population screen — see CLAUDE.md §Gotchas). **Phase 9.3 gate-C1 fix MERGED as #681** (branch `claude/phase-9-3-precache-broad-split`, 2026-07-02) — a single-job cold `universe: broad_investable_us` dispatch was CANCELLED at GitHub Actions' hard 6-hour per-job ceiling with no cache saved; fix is a new `.github/workflows/precache-broad-universe.yml` 4-job sequential split (each job ≤ 360 min) + a prerequisite `compute/orchestrator/history.py` extraction (Step 3 annual-history loop, pure code move) + `compute-rankings.yml`/`precache-edgar.yml` timeout corrections (both were set ABOVE GHA's unmovable 360-min hard cap). NO schema bump; default sp1500 cron path BYTE-IDENTICAL. Full entry + verification ladder in PHASE_STATUS_INFLIGHT.md; gotcha detail in CLAUDE.md §Gotchas. Nothing else in flight as of 2026-07-02 (the #616 sim-pin above is a standing configuration guard, not a pending PR).

Current schema **`0.10.44-phase9pilot`** on `main` (#684 squash `684e36ec`, merged 2026-07-02 — `broad_investable_us` universe-floor scoping PR-1: `BROAD_UNIVERSE_ADV_FLOOR_USD` decouples the RANKED-path investability floor from the sp1500 `low_liquidity` floor + shadow `sweep_broad_universe_floors` 6-field survivor sweep; financial-engineer→literature-searcher (Fama-French 2008 + Hou-Xue-Zhang 2020)→methodology-scientist RATIFY-WITH-CONDITIONS; SHADOW-only, rankings byte-identical, defense UNCHANGED at 38; +33 tests). Prior **`0.10.43-phase9pilot`** (#652 squash `135bf9f7`, merged 2026-06-29 — `restatement_history` manipulation-index weight demotion, issue #16: weight 5.0→0.0 + `restatement_high_confidence` 3.0→8.0 preserving the irregularity total; RANK-NEUTRAL, rankings byte-identical; defense UNCHANGED at 38). Prior **`0.10.42-phase9pilot`** (#665 squash `ca3b1f06`, merged 2026-06-29 — Phase 9.1 Broad Investable US universe coverage probe, Rule 18 obs-first: 6 new write-only `Metadata.broad_universe_*` fields; wired unconditionally into the weekday cron same-day by #667 Phase 9.1b; rankings byte-identical; defense UNCHANGED at 36, reconciled to 38 by #685's doc-truing). Prior **`0.10.41-phase8pilot`** merged 2026-06-27: **#631** (feat(compute): Option-B dividend pool-and-redeploy SHADOW NAV, issue #620 — `nav.adaptive_div_pooled` artifact series + 2 canary `Metadata` fields `div_pool_shadow_terminal_nav_delta_pct` / `div_stream_coverage_pct` (the ONLY two in the triple; A/B-diff `meta.div_pool_*` on the artifact — C-2/E artifact-vs-triple split); `compute/portfolio/backtest.py` `dividends`/`price_basis` kwargs + BYTE-IDENTICAL guard; `compute/ingest/prices.py` `actions=True` + `fetch_dividends_panel` (`QR_SKIP_DIVIDENDS=1`); SHADOW/obs-first Rule 18 — live `nav.adaptive`/headline/rankings BYTE-IDENTICAL; backfill-verify Option B −1.03% net vs Option A over ~10y, coverage 80.6%, Carino 4.7e-16; pre-merge-prod-sim ranking diff adjudicated DATA-DRIFT not code; headline flip = SEPARATE future PR + owner sign-off; schema `0.10.40`→`0.10.41-phase8pilot`; defense UNCHANGED at 36 — squash `858bec01`) · **#632** (feat(frontend): drop per-row TWR `% price` + `since` sub-lines from the Current-picks return cell — keep only the headline MWR "Your return"; frontend-only, NO schema bump; rankings byte-identical — squash `d772f66d`). Prior: the legendary-fund 6-proposal deep-research program (`claude/fund-performance-rankings-f8x4o1`, PRs #604/#607/#615/#617/#624/#628 — Proposals F/D/A/C-2/C-1/E, all SHADOW/obs-first) merged complete 2026-06-26 (schema chain `0.10.33`→`0.10.40`); **Proposal B (PEG/GARP) REJECTED** (φ(PEG,P/E)=0.849 > 0.5 orthogonality gate). Deferred to Q3 2026-08-19 cohort audit / future owner-signed PR: #620 dividend headline-flip (Option A→B) · `low_liquidity` veto promotion (#544) · Bonferroni provisional-threshold re-derivation (#654, split from the closed #542). See PHASE_STATUS_INFLIGHT.md.
Prior schema chain on `main` (before the current `0.10.41-phase8pilot` — see §In-flight above): **`0.10.40-phase8pilot`** (#628 squash `eb20b005`, merged 2026-06-26 — legendary-fund Proposal E turnover/hysteresis + liquidity capacity tilt SHADOW; the 6-proposal program `0.10.33`→`0.10.40` all SHADOW/obs-first, defense UNCHANGED at 36); prior **`0.10.33-phase8pilot`** (#588 squash `d3058434`, merged 2026-06-24 — two-factor `value_trap_risk` shadow counter (issue #586): new `Metadata.value_trap_risk_two_factor_shadow_count: int | None` (LSV 1994 second leg — trailing P/E below sector-peer median, layered on the Penman `ROE ≤ Ke` gate); SHADOW/Rule-18, live `valuation_warnings` byte-identical, defense UNCHANGED at 36); prior **`0.10.32-phase8pilot`** (#590 squash `54cb5bcb`, merged 2026-06-24 — Rule-18 counter `Metadata.extreme_estimate_majority_lowapp_count` for the `extreme_estimate_majority` low-applicability floor; annotate-only, defense UNCHANGED at 36); prior **`0.10.31-phase8pilot`** (#565 squash `2c9dc1371`, merged 2026-06-22 — Security-type (Type) HeroAttributeTile ingest PR-1 (issue #541): `StockDetail.security_type` from yfinance `fast_info.quote_type` + `Metadata.security_type_coverage_pct` coverage canary; obs-first Rule 18, NO UI wiring (Type tile stays 'Coming soon'); display-only, rankings byte-identical; defense UNCHANGED at 36; +17 tests); prior **`0.10.30-phase8pilot`** (#564 squash `62dbf4f89`, merged 2026-06-22 — Bonferroni multi-test shadow counter (Slice 8, #542): 3 new `Metadata.bonferroni_shadow_*` fields, `compute/scoring/bonferroni_shadow.py`, SHADOW/OBSERVABILITY-ONLY, `m = valid_count`, provisional threshold −1.94 placeholder, defense UNCHANGED at 36, 20 tests); prior **`0.10.29-phase8pilot`** (#527 squash `2e45a33bf`, merged 2026-06-20 — S&P 1500 cutover Slice 4: `low_liquidity` ANNOTATE flag (<$5M ADV, Amihud 2002; rank-neutral — `valuation_warnings`, not `risk_flags`) + `compute_average_dollar_volume()` + `StockDetail.average_dollar_volume` + `Metadata.low_liquidity_annotate_count`; defense 35→36 (new annotate); rankings/scores byte-identical; dormant on sp900, lights up on sp600; methodology RATIFY-SHADOW, veto promotion deferred); prior **`0.10.28-phase8pilot`** (#519 squash `5e49dca0a`, merged 2026-06-20 — S&P 1500 cutover Slice 2: `sp1500` universe seam + small-cap coverage probe; 3 new `Metadata.smallcap_*` fields; sp600 PROBE-ONLY (label `SP1500-probe`, NOT ranked); defense UNCHANGED at 35); prior **`0.10.27-phase8pilot`** (#512 squash `78fd608423`, merged 2026-06-20 — Dividend signal PR-1: 3 new `StockDetail` fields `dividend_yield_pct`/`pays_dividend`/`payout_ratio` + `Metadata.dividend_coverage_pct` coverage canary; `_yf_info_fetch` 2-tuple → 4-tuple; rankings/scores/flags byte-identical; defense UNCHANGED at 35; +25 tests); prior **`0.10.26-phase8pilot`** (#501 squash `72ee8667d`, merged 2026-06-19 — cross-source share-count-corruption SHADOW observability PR-1; 4 new `Metadata.cross_source_corruption_*` fields; MUTATES NOTHING, defense UNCHANGED at 35). Recent no-schema-bump parallel merges (tracker-only; schema stays `0.10.29`): #514 (S&P 1500 scout) · #515 (R6 weekend-test fix) · #517/#522 (Vercel analytics) · #518/#525/#528 (pytest-cov + ingest coverage tests) · #520 (S&P 1500 Slice 5 — `cache-v11-fast` precache bump + `sp1500` dispatch; cron default was unchanged at #520 time, later flipped to sp1500 by #534) · #521 (UI polish) · #524 (Mode C) · **#531** (S&P 1500 Slice 6 — `SmallcapChip` + SML tab activation; frontend-only, NO schema bump) · **#534** (S&P 1500 cutover **Slice 7** — cron-default flip `sp900`→`sp1500` across `compute-rankings.yml` + `precache-edgar.yml` defaults + `pre-merge-prod-sim.yml` pin; `compute/main.py` lifts the Slice-2 probe-only sp600 filter so the weekday cron RANKS the full ~1504 names (`Metadata.universe` `SP1500-probe`→`SP1500`); cohort-size recompute gate widened to `in ("sp900","sp1500")`; smallcap coverage probe + russell1000-proxy sp600 guard retained; NO schema bump (stays `0.10.29`); defense UNCHANGED at 36; validation 1504 names / cold ~174 min (< 240 ceiling) / warm ~45 min (< 90) / smallcap coverage 99.67% — squash `8301b82cb`, merged 2026-06-21; the 902→~1504 production cutover) · **#533** (fix(ingest): `dividend_yield_pct` ×100 double-scaling removal — yfinance `.info["dividendYield"]` already returns percent; the #512 `×100` over-scaled 100×; adds a `>100` format-reversion guard; NO schema bump, display-field-only, rankings byte-identical) · **#537** (feat(frontend): append rotated-out "Sold" rows to the Current-picks table; frontend-only, NO schema bump) · **#548** (feat(frontend): infinite-scroll the ~1500-row ranking table, Slice 8 §8.3 frontend gate) · **#549** (feat(frontend): wire the HeroAttributeTiles Dividend tile to real data, PR-2) · **#539/#547/#570** (research warehouse Slices 1/2 — per-run PIT Parquet writer + maximum-history backfill + per-method `fp_*` dtype fix) · **#552** (drop free-text post-split valuation_warning + DQIC docstrings) · **#553** (timeout-minutes 240→270 for S&P 1500 scale) · **#554** (payout_ratio >20→None guard) · **#555** (XBRL balance-sheet tag selection — instant + 10-K/Q form filter; fixes HASI/LGIH/GPK) · **#556/#557/#559/#560/#575** (dependabot + CI Node 20→22) · **#596** (feat(frontend): Current-picks Return column + Performance `<tfoot>` Total-return footer — Score→Return adaptive branch, redundant Change column removed, `entryCloses`/`lastCloses` extended to prior-basket tickers for sold-row returns; display-only, NO schema change — squash `ca4b95a5`). **Latest tag:** **`v2.0.0-phase8` PUBLISHED 2026-06-23** at `8c89a5af0` ("Set as latest"; release PR #577 merged 2026-06-22 + the green scheduled sp1500 cron it gated on — Section A-L 12/12, defense 36). The S&P 1500 cutover epic is COMPLETE. `low_liquidity` veto promotion (#544) — methodology-scientist RATIFY-WITH-CONDITIONS 2026-06-23; **PRE-REGISTERED (docs-only) for the Q3 2026-08-19 cohort audit**: keep the $5M annotate + add a stricter `ADV_VETO_FLOOR_USD=$3M` veto floor (flips only BFS/CENT/SBSI), gated on acceptance bands B1-B5 (see `docs/METHODOLOGY.md` §low_liquidity + `compute/config.py`); investability veto, not an alpha claim → owner sign-off. Bonferroni provisional-threshold re-derivation (#654, split from the closed #542) still deferred post-v2.0. See CLAUDE.md §Phase status + PHASE_STATUS_INFLIGHT.md.
...
fires post-cron + pre-release + "ตรวจ data หุ้น"). Newest Tier 4
addition: `loop-engineer` — the work-loop architect that, given an
assigned task, DESIGNS the Goal → Context → Action → Check/Fix →
...
on the sonnet pool (21 of 27 agents) should walk every relevant
