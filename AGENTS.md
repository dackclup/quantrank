...
preserves it).
**In flight**: **`restatement_history` weight demotion (issue #16)** — DEMOTE the bare annotate's manipulation-index weight 5.0 → 0.0 (over-fires 17.82% on SP1500 vs HLM 2008 1-3%; fraud-class content carried by `restatement_high_confidence`, raised 3.0 → 8.0 to preserve the irregularity total), new shadow `Metadata.restatement_history_weight_demote_delta_count`; schema `0.10.42-phase9pilot` → **`0.10.43-phase9pilot`**; RANK-NEUTRAL / live rankings byte-identical; defense UNCHANGED at 38 (DEMOTE not DEPRECATE); see PHASE_STATUS_INFLIGHT.md. Also in flight: Phase 9.1 Broad Investable US universe coverage probe (branch `claude/phase-9-1-broad-investable-probe`, schema `→0.10.42-phase9pilot`) — HARD NAMING CONSTRAINT: "Broad Investable US" everywhere — NEVER "Russell 3000" / "Russell-3000-class" / "equivalent to Russell 3000"; HARD RULE 18: 6 new `broad_universe_*` Metadata fields MUST NEVER be read by scoring, composite, pillar, veto/flag, fair-price, or `select_picks`.
