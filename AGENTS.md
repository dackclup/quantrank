...
preserves it).
**In flight**: **#616 simulate runner-ceiling** — pin the `pre-merge-prod-sim` `QR_UNIVERSE` to **sp500** (one-line) while the cron still ranks sp1500: a cold sp1500 sim (~3.6h) trips the ~4h hosted-runner ceiling; sp500 cold (~43min) still fetches live (real diff on the sp500 ∩ committed-main intersection) but stays under it. The skip-live-fetch fix (Option 4a, `QR_SIM_NO_LIVE_FETCH`) was REVERTED (full cold miss → 0 tickers priced < `MIN_VALID_TICKERS` → compute abort, no output). CI/test/docs only; rankings byte-identical; defense UNCHANGED at 38; guarded by `test_sim_pins_sp500_universe_per_616_exception`. See PHASE_STATUS_INFLIGHT.md. Also in flight: Phase 9.1 Broad Investable US universe coverage probe DRAFT PR (branch `claude/phase-9-1-broad-investable-probe`) + Phase 9.1b cron-wiring DRAFT PR (branch `claude/phase-9-1b-cron-wiring`, depends on 9.1) — schema `→0.10.42-phase9pilot` (DRAFT). Phase 9.1 ships the probe definition + schema fields; Phase 9.1b adds `QR_SKIP_BROAD_UNIVERSE=1` to `pre-merge-prod-sim.yml` + cron wiring (CLAUDE.md §Gotchas notes the probe runs on the weekday cron, reusing the SP1500 `prices_by_ticker` dict, ~40–45% coverage expected). Rankings/scores/flags BYTE-IDENTICAL; defense UNCHANGED at 38. HARD NAMING CONSTRAINT: "Broad Investable US" everywhere — NEVER "Russell 3000" / "Russell-3000-class" / "equivalent to Russell 3000"; HARD RULE 18: 6 new `broad_universe_*` Metadata fields MUST NEVER be read by scoring, composite, pillar, veto/flag, fair-price, or `select_picks`.
