...
preserves it).
**In flight**: Phase 9.1 Broad Investable US universe coverage probe DRAFT PR (branch `claude/phase-9-1-broad-investable-probe`) — see `PHASE_STATUS_INFLIGHT.md` for full entry. Schema `0.10.41-phase8pilot` → **`0.10.42-phase9pilot`** (DRAFT, not yet merged). Rankings/scores/flags BYTE-IDENTICAL; defense UNCHANGED at 36. HARD NAMING CONSTRAINT: "Broad Investable US" everywhere — NEVER "Russell 3000" / "Russell-3000-class" / "equivalent to Russell 3000". HARD RULE 18: 6 new `broad_universe_*` Metadata fields MUST NEVER be read by scoring, composite, pillar, veto/flag, fair-price, or `select_picks`.
