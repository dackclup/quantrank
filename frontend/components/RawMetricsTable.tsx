...
export default function RawMetricsTable({ metrics }: { metrics: RawMetrics }) {
  return (
    <div className="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-medium dark:border-slate-800 dark:bg-slate-900">
      <table className="min-w-full divide-y divide-slate-200 text-sm dark:divide-slate-800">
        ...
        <tbody className="divide-y divide-slate-100 dark:divide-slate-800/60">
          {ROWS.map(({ key, label, fmt, hint }) => {
            ...
            return (
              <tr key={key} className="odd:bg-white even:bg-slate-50 hover:bg-slate-100 dark:odd:bg-slate-900 dark:even:bg-slate-900/50 dark:hover:bg-slate-800">
