...
// Cumulative total-return table: one row per calendar year showing the
// portfolio's and benchmark's CUMULATIVE return since inception (not
// year-over-year). Footer row shows the full-window total return.
// Structurally mirrors AnnualReturnsTable (same Props, helpers, and
// design tokens) so the two tables read as one family side-by-side.
...
// year -> NAV at the LAST finite date in that year (dates are chronological,
// so the final write per year wins).
...
// Cumulative total return since inception for each calendar year:
// value = NAV(year-end) / NAV(inception) − 1.
// The first year in the window is based off the inception NAV, so it is the
// partial-year total return since inception, flagged `partial` by the caller.
function cumulativeReturns(
  ...
  const inception = firstFin(series);
  for (const y of [...ye.keys()].sort((a, b) => a - b)) {
    const end = ye.get(y) as number;
    out.set(y, inception !== null && inception !== 0 ? end / inception - 1 : null);
  }
  return out;
}
...
export function TotalReturnTable({
  dates,
  portfolio,
  benchmark,
  portfolioLabel,
  ...
  const { rows, totalPortfolio, totalBenchmark, windowYears } = useMemo(() => {
    if (dates.length === 0) {
      return { rows: [] as Row[], totalPortfolio: null, totalBenchmark: null, windowYears: 0 };
    }

    const pCum = cumulativeReturns(dates, portfolio);
    const bCum = cumulativeReturns(dates, benchmark);
    const years = [...pCum.keys()].sort((a, b) => a - b);
    const firstYear = years[0];
    const startsMidYear = Number(dates[0].slice(5, 7)) > 1;

    const rows: Row[] = years.map((y) => ({
      year: y,
      portfolio: pCum.get(y) ?? null,
      benchmark: bCum.get(y) ?? null,
      partial: y === firstYear && startsMidYear,
    }));

    // Full-window cumulative total return: lastFin / firstFin − 1.
    const totalReturn = (series: (number | null)[]): number | null => {
    ...
    return {
      rows,
      totalPortfolio: totalReturn(portfolio),
      totalBenchmark: totalReturn(benchmark),
      windowYears: yrs,
    };
  ...
  return (
    <div className="rounded border border-slate-200 bg-white p-4 shadow-subtle dark:border-slate-800 dark:bg-slate-900 md:p-6">
      ...
      <p className="mb-3 text-xs text-slate-500 dark:text-slate-400">
        Cumulative backtest return since inception — {portfolioLabel} (net of cost) vs {benchmarkLabel}.
      </p>
      ...
      <table className="w-full text-sm">
        ...
        <tfoot>
          <tr className="border-t-2 border-slate-200 bg-slate-50 dark:border-slate-700 dark:bg-slate-800/50">
            ...
            <td className={`py-2 text-right font-mono text-base font-bold tabular-nums ${tone(totalPortfolio)}`}>
              {pct2(totalPortfolio)}
            </td>
            <td className={`py-2 text-right font-mono font-semibold tabular-nums ${tone(totalBenchmark)}`}>
              {pct2(totalBenchmark)}
            </td>
          </tr>
        </tfoot>
      ...
      <p className="mt-2 text-pretty text-xs leading-relaxed text-slate-500 dark:text-slate-400">
        Total return = cumulative growth over the {windowYears.toFixed(1)}-year backtest window
