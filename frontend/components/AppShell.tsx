  ...
  return (
    <div className="flex min-h-screen flex-col">
      {/* Full-width top chrome — the controls + tab nav + the market-stats strip
          span edge-to-edge ABOVE the sidebar, so the sidebar never eats into the
          top bar (only into the content area below it). */}
      <header className="sticky top-0 z-20 border-b border-slate-200 bg-white/95 backdrop-blur dark:border-slate-800 dark:bg-slate-950/95">
        <div className="flex h-14 items-center gap-3 px-4 md:px-6">
          ...
          {/* Desktop brand — Q + wordmark live in the TOP header ONLY while the
              sidebar is COLLAPSED; hidden when expanded (brand lives in the rail)
              and on mobile (the md:hidden wordmark covers it). `data-rail="show"`
              renders it from the first paint on a collapsed refresh (no flash). */}
          <Link
            href="/"
            data-rail="show"
        ...
        {/* Primary tab nav (Home · Ranking · News · Analysis · Portfolio) — a
            second header row, sticky with the controls above it. Coexists with
            the left-rail Sidebar (user chose "keep both"). */}
        <div className="border-t border-slate-200 dark:border-slate-800">
          <TopNav />
        </div>
      </header>
      {/* Universe-snapshot ticker strip (server-rendered, passed in from the
          layout) — full-width, below the sticky header, scrolls away. */}
      {topBar}
      {/* Sidebar (left rail / mobile drawer) + page content. The desktop rail
          sticks BELOW the full-width top chrome (its md:top offset = the header
          height) so it only eats into the content width, never the top bar. */}
      <div className="flex flex-1">
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          ...
          {/* Top padding kept tight (pt-4/pt-6) so the page content sits higher
              under the sticky header. Bottom padding stays roomy so content
