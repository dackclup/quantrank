// Shared display helpers for the AI-pick portfolio views.
...
//
// Extracted in PR-2b (MWR/Carino return redesign) — previously
// AiPickPortfolio.tsx inlined apportionWeightLabels, pctStr, toneClass.
// HoldingsTimeline.tsx now imports them from here instead of duplicating.
 ...
 * honestly tracks the finite-weight sum (so a genuinely <100% book would still
 * show <100%, just without per-row rounding noise).
 */
 ...
 * Uses the typographic minus (U+2212) to match the MWR / Return column spec.
 */
 ...
 * Matches the design-system soft palette (Rule 1 + Rule 4 dark pairs).
 */
...
/**
 * Muted tone for the TWR "Stock price return" shadow line.
 * Uses a slightly less prominent shade than the MWR headline.
 */
export function twrToneClass(v: number | null): string {
