...
def test_fundamentals_one_returns_tuple_with_elapsed(monkeypatch):
    ...
    from compute.ingest.fundamentals import fetch_fundamentals as orig
    sentinel = _snap()

    def fake(*a, **k):
        return sentinel

    monkeypatch.setattr("compute.orchestrator.fundamentals.fetch_fundamentals", fake)
...
def test_fundamentals_one_failure_returns_none_and_elapsed(monkeypatch):
    ...
    monkeypatch.setattr("compute.orchestrator.fundamentals.fetch_fundamentals", boom)
    snap, elapsed = _fundamentals_one("TST", "0000000001")
    assert snap is None
    assert elapsed >= 0.0
