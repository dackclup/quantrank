...
## PR (branch `claude/nasdaq-russell-indices-da25zv`) — Phase 9.4 PR-1: broad-universe sector-resolution coverage canary (in flight, 2026-07-02, schema `0.10.45-phase9pilot`)

OBSERVABILITY-ONLY (Rule 18) first slice of Phase 9.4 (the owner-chosen "source GICS sector before publishing the Broad Investable US tab" path). The `broad_investable_us` RANKED path sets `sector="Unknown"` for every candidate (SEC company-tickers source has no classification field) → flat 10% Ke + a single heterogeneous "Unknown" peer bucket → methodology-scientist RATIFIED that fair-price must be suppressed-to-None for those names, so the only way to give the ~2,000 sector-less names a real fair-price is to give them a real sector. data-pipeline-engineer scoped yfinance `.info["sector"]` as the near-zero-cost source: the Step-8 per-ticker loop already calls `fetch_yfinance_market_cap` → `_yf_info_fetch` (one `.info` round-trip) for every scored ticker regardless of universe, so sector is a free side-channel read off the already-warm `yfinance_info` cache (the 4th instance of the dividend #512 / security_type #565 warm-cache-piggyback pattern).

This PR MEASURES sector resolvability; it does NOT change any sector/Ke/fair-price/sector-median/ranking/score (wiring the resolved sector into `candidates_to_universe_frame` is the deferred PR-2, gated on ≥1 real dispatch confirming the canary + financial-engineer Ke sign-off + methodology-scientist confidence-gate). Changes: `_yf_info_fetch` widened 4-tuple → 6-tuple (adds `sector`, `industry`; the ONE production unpack site `fetch_yfinance_market_cap` updated); new `fetch_yfinance_sector` pure cache-read + `_YF_SECTOR_TO_GICS` 11-entry map + `map_yfinance_sector_to_gics` (5 verbatim + 6 renames: Basic Materials→Materials, Consumer Cyclical→Consumer Discretionary, Consumer Defensive→Consumer Staples, Financial Services→Financials, Healthcare→Health Care, Technology→Information Technology; unmapped → None); `_yf_sector_resolved_by_ticker` accumulator in the per-ticker loop (`PerTickerLoopResult` 28→29 fields); a `QR_UNIVERSE == "broad_investable_us"`-gated aggregation in `main.py` (own try/except → None) emitting the new `Metadata.broad_universe_sector_resolved_pct: float | None`. Schema triple lockstep + `SCHEMA_VERSION` `0.10.44-phase9pilot` → `0.10.45-phase9pilot`.

Rule-18 write-only (grep-confirmed by quantrank-reviewer + defense-layer: the field/map/reader are NEVER read by `compute/scoring/**` or `compute/valuation/**`); rankings/scores/flags BYTE-IDENTICAL on every path; defense layer UNCHANGED at 38; zero new network round-trips. A permanent ratchet test pins `set(_YF_SECTOR_TO_GICS.values()) == set(SECTOR_COST_OF_EQUITY.keys())` (error→regression: a future edit to either taxonomy that breaks the mapping fails CI). Biggest risk = yfinance sector-string drift (dividend #533 / payout #554 class) — the coverage canary is the drift tell. Design provenance: financial-engineer Phase 9.4 scope → methodology-scientist RATIFY (separate-artifact `rankings_broad.json` locked + suppress-fair-price-on-unresolved-sector) → owner "source GICS sector first" → data-pipeline-engineer sourcing scope (yfinance piggyback) → compute-builder + test-engineer.

**Files**: `compute/ingest/cross_source.py`, `compute/orchestrator/per_ticker.py`, `compute/main.py`, `compute/output/schemas.py`, `compute/config.py`, `frontend/lib/types.ts`, `frontend/lib/schema-snapshot.json`, `tests/test_ingest/test_cross_source_sector.py` (new), `tests/test_ingest/test_broad_universe_sector_canary.py` (new), `tests/{test_config.py,test_ingest/test_cross_source.py,test_ingest/test_cross_source_dividend.py,test_orchestrator/test_per_ticker.py}` (pin updates), `PHASE_STATUS_INFLIGHT.md` (this entry).

**Verification**: `ruff check .` clean · `pytest -m "not network"` 3519 passed / 0 failed (+38: 9 pin-fixes + 29 new) · `python -m compute.output.schema_check` in sync · quantrank-reviewer (opus) PASS on all substantive invariants (Rule 18 write-only, tuple-widen safety, taxonomy ratchet, schema triple, HARD naming, defense 38, zero round-trips) · schema-sentinel PASS. On-merge housekeeping: add the `0.10.45-phase9pilot` row to the SKILL.md schema-version table + refresh the CLAUDE.md `_yf_info_fetch` 4-tuple→6-tuple §Gotchas note.
