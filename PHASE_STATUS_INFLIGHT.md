...
## PR #259-R6 — orchestrator refactor R6 (Phase-4h OSAP signal-replication block) (in flight, 2026-07-01)

**Branch**: `claude/orchestrator-refactor-r6`. Sixth slice of the 7-PR incremental refactor of `run_weekly_compute` (issue #259). The most output-heavy slice so far (~10 accumulators) — MEDIUM-HIGH risk per the task brief; verified exact.

**Scope (PURE CODE MOVE — byte-identical output)**: R6 extracts the entire Phase-4h OSAP signal-replication + PBO/DSR gate + Path-b blend block (the `# Phase 4h — OSAP signal replication…` comment through the outer `except Exception as e:` reset block, ~lines 1889-2091 in pre-R6 main.py) out of `run_weekly_compute` into a new `compute/orchestrator/osap.py` module, as a single function `run_osap_pipeline(pillar_df, composite, asof_date) -> OsapPipelineResult`. No logic change, no reordering of effects, same deferred-import placement (Phase-4a pattern), same wall-clock, same log text/levels — output byte-identical across both the happy path and the outer-except path.

**Scope boundary respected**: the sibling Phase-4j.1 Qlib Alpha158 block (~line 2093+ in pre-R6 main.py, its own `alpha158_*` accumulators) and any IPCA block are SEPARATE observability surfaces with their own accounting — NEITHER was touched. Verified by reading the Alpha158 block in full (lines 2093-2260ish) and confirming it reads only `asof_date` / `pillar_df` (both pre-existing locals, unrelated to this extraction) plus the shared `OsapGateDiagnostic` schema class — it references NO `osap_*` variable name defined by the extracted block.

**The ~10 outputs** (validated by grepping every candidate name's uses in pre-R6 main.py and confirming each is both assigned inside the block AND read AFTER it, feeding either the `Metadata` constructor at ~line 3921-3930/4065 or the per-ticker `StockDetail.osap_blended_score` derivation at ~line 3167-3171): `osap_signals_used` · `osap_excluded_signals` · `osap_signals_ic_12m` · `osap_signal_map` · `osap_signals_coverage_pct` · `composite_osap_adjusted` (a `pd.Series`) · `osap_signals_missing_from_dataset` · `osap_gate_diagnostics` · `osap_signals_dropped_no_long_short_list` · `osap_wall_clock_seconds`. Returned via a `NamedTuple` (`OsapPipelineResult`) rather than a bare 10-tuple, per the task's explicit guidance (10 positional unpacks is error-prone) — `compute.main` unpacks the call result into the same 10 local names via a parenthesised tuple-target assignment.

**Inputs (explicit function parameters)**: `pillar_df` (only `.index` is read — the ticker list passed as `tickers=` to `compute_osap_signals`), `composite` (the raw composite-score `pd.Series`, read-only — the blend input to `apply_osap_blend`), `asof_date` (the run's as-of `date`, passed to `fetch_osap_returns` + `compute_osap_signals`). All three are pre-existing locals assigned well before the block (`pillar_df` at pre-R6 main.py:1598, `composite` at :1770, `asof_date` at :1849) and confirmed NOT mutated in-place anywhere inside the block (only read via `.index`, `.isoformat()`, or passed as a plain function argument) — clean parameterization, no shared-mutable-state footgun.

**Deferred imports** (Phase-4a pattern, preserved verbatim inside the new module's function): `compute.features.osap_replicate` (`compute_long_short_returns` / `compute_osap_signals` / `coverage_by_signal` / `signals_dropped_no_long_short` / `signals_in_dataframe`), `compute.ingest.osap` (`fetch_osap_returns`), `compute.scoring.osap_blend` (`aggregate_osap_signals` / `apply_osap_blend`), `compute.validation.osap_validation` (`compute_rolling_ic_12m` / `filter_accepted_signals` / `gate_osap_signals`) — all four import statements stay INSIDE the try block in `run_osap_pipeline`, exactly as before, because `compute.ingest.osap` pulls in `openassetpricing` at module load (only installed via the `.[factors]` optional extra) and a top-level import would break base-install test collection. Moving these to the new module's top level would defeat the whole point of the deferred-import pattern, so they were NOT moved — only relocated from "inside `run_weekly_compute`'s try block" to "inside `run_osap_pipeline`'s try block", same relative position.

**`OsapGateDiagnostic` import disposition — KEPT in `compute.main`**: this schema class (`compute/output/schemas.py`) is shared between the extracted OSAP block AND the sibling Alpha158 block (`alpha158_gate_diagnostics: dict[str, OsapGateDiagnostic] = {}` at pre-R6 main.py:2113, still inline and untouched). Since Alpha158 still constructs `OsapGateDiagnostic` instances directly, `compute.main`'s top-level `from compute.output.schemas import (…, OsapGateDiagnostic, …)` import was left UNCHANGED — removing it would break the Alpha158 block. The new `compute/orchestrator/osap.py` module ALSO imports `OsapGateDiagnostic` directly (for its own `osap_gate_diagnostics: dict[str, OsapGateDiagnostic]` return-type annotation) — this is an independent import in the new module, not a relocation.

**What moved (verbatim, not altered)**: the entire Phase-4h block — all ~10 accumulator inits (empty lists/dicts, `composite_osap_adjusted: pd.Series = pd.Series(dtype=float)`), the `_osap_wc_start = time.monotonic()` wall-clock marker, the `try:` with its 4 deferred imports, the `fetch_osap_returns(signals=list(config.OSAP_SIGNALS_100), as_of=asof_date)` fetch, the manifest-vs-dataset silent-drop diff (`osap_signals_missing_from_dataset`), the no-long-short-pair diff restricted to the manifest (`osap_signals_dropped_no_long_short_list`), `compute_long_short_returns`, the PBO/DSR gate call (`gate_osap_signals(osap_ls, requested_signals=config.OSAP_SIGNALS_100)`), the per-signal `OsapGateDiagnostic` construction into `osap_gate_diagnostics`, `filter_accepted_signals`, the rolling-12m IC loop (`compute_rolling_ic_12m`, `round(float(ic), 4)`, `if ic is not None` guard), the `if osap_signals_used:` branch building `osap_signal_map` / `osap_signals_coverage_pct` / the Path-b blend (`aggregate_osap_signals` → `apply_osap_blend(composite, osap_aggregate)` → `composite_osap_adjusted`), the `else:` zero-accepted-signals warning log, the wall-clock end marker (`round(time.monotonic() - _osap_wc_start, 1)`), and the outer `except Exception as e:` that resets all ~10 outputs to their empty/None/`pd.Series(dtype=float)` values with the identical warning-log text. All log lines' text/levels/interpolation (`%d`/`%s`) preserved exactly.

**`compute/main.py` changes**:
- Added `from compute.orchestrator.osap import run_osap_pipeline` import (alongside the existing `form4`/`fundamentals`/`prices`/`tier2` orchestrator imports, alphabetically ordered).
- Removed the ~204-line OSAP block (pre-R6 main.py lines 1889-2091, plus the leading comment line 1889 itself — replaced entirely).
- Replaced it with an 18-line block: a short comment pointing at the new module + a parenthesised tuple-unpack assignment `(osap_signals_used, …, osap_wall_clock_seconds,) = run_osap_pipeline(pillar_df, composite, asof_date)`.
- `OsapGateDiagnostic` import: UNCHANGED (still needed by the untouched Alpha158 block — see disposition above).
- `time` top-level import: UNCHANGED (still used by the Alpha158 block's own wall-clock + a separate cross-source wall-clock block, both untouched).
- Everything downstream (the `osap_*` values flowing into the `Metadata` constructor with the identical field names, and `composite_osap_adjusted` → per-ticker `osap_blended_score` derivation in the Step-8 loop) is completely untouched and reads the same 10 locals, populated identically.

**Byte-identical guarantee**: happy-path populates all ~10 outputs identically to the inline block (same fetch → manifest-diff → gate → IC-loop → signal-map/coverage → Path-b blend sequence, same log lines); the zero-accepted-signals branch skips the signal-map/blend exactly as before (leaving `composite_osap_adjusted` at its `pd.Series(dtype=float)` init — `apply_osap_blend` never called); the outer-except resets all 10 to `[]`/`[]`/`{}`/`{}`/`{}`/`pd.Series(dtype=float)`/`[]`/`{}`/`[]`/`None` with the identical warning text. Verified via a line-by-line diff against `git show HEAD:compute/main.py` before writing the new module, plus a real end-to-end integration-test confirmation (see below).

**Existing tests — no update needed, verified via the real integration harness**: grepped `tests/` for any `patch("compute.main.<osap-symbol>", …)` reference to the moved symbols (`fetch_osap_returns` / `compute_long_short_returns` / `compute_osap_signals` / `aggregate_osap_signals` / `apply_osap_blend` / `gate_osap_signals` / `filter_accepted_signals` / `compute_rolling_ic_12m` / `coverage_by_signal` / `signals_in_dataframe` / `signals_dropped_no_long_short`) — NONE found; these functions were never directly monkeypatched at the `compute.main` level anywhere in the suite. `tests/test_output/test_wall_clock_schema.py::test_osap_wall_clock_none_on_pipeline_failure` DOES drive `run_weekly_compute` end-to-end via the `_run_orchestrator` harness and simulates an OSAP failure — but it does so via a `monkeypatch.setitem(sys.modules, "compute.ingest.osap", fake_osap)` injection, which patches `sys.modules` directly rather than an attribute on a specific caller module. This technique is location-agnostic: the deferred `from compute.ingest.osap import fetch_osap_returns` picks up the fake module regardless of whether that deferred import lives in `compute.main` or `compute.orchestrator.osap`. Re-ran this exact test post-refactor — **PASSED**, confirming byte-identical outer-except behaviour through the real harness with zero test-file changes required. `tests/test_output/test_schema_phase4h2.py` and the two other `osap_wall_clock_seconds`/`osap_gate_diagnostics` references in that same file test the `Metadata` Pydantic schema directly (constructing `Metadata(**payload)`), not `run_weekly_compute`'s internals — unaffected by this refactor.

**`compute/scoring/**`, `compute/ingest/**`, `compute/valuation/**`, `compute/features/**`, `compute/validation/**`**: NOT modified (esp. `compute/ingest/osap.py`, `compute/features/osap_replicate.py`, `compute/scoring/osap_blend.py`, `compute/validation/osap_validation.py` — the four OSAP-library modules the deferred imports pull from — and the Alpha158/IPCA blocks, all untouched). **Schema triple**: untouched — `schemas.py` / `types.ts` / `schema-snapshot.json` not modified; `schema_check` reports in-sync (no drift, as expected for a pure code move).

**Tests**: 17 new offline unit tests in `tests/test_orchestrator/test_osap.py` (21 collected pytest IDs — one test, `test_C1_exception_anywhere_in_try_resets_all_ten_outputs`, is `@pytest.mark.parametrize`'d over the 5 distinct steps that can raise inside the try block, expanding 1 `def` into 5 collected cases, mirroring R4's `test_D1_D2_D3_skip_env_var_gives_all_empty_none` precedent) — cover `run_osap_pipeline` happy-path (A1-A9: NamedTuple shape, accepted/excluded signals, rolling-IC population incl. the `if ic is not None` skip guard, signal-map/coverage population, the exact blend Series identity, manifest-vs-dataset diff, gate-diagnostics keying, no-long-short-list manifest restriction, non-negative wall-clock float), the zero-accepted-signals branch (B1-B2: blend never invoked + `composite_osap_adjusted` stays empty, warning logged mentioning "0 signals"), the outer-except path parametrized over 5 different raise points (C1) plus a warning-log check (C2) and an explicit `ImportError`-is-caught-the-same-way case (C3), and return-shape identity guards (D1-D2: wall-clock is `None` not `0.0` on failure, `composite_osap_adjusted` is always a `pd.Series` on both paths). Every deferred-import function is monkeypatched at its SOURCE module (`compute.features.osap_replicate` / `compute.ingest.osap` / `compute.scoring.osap_blend` / `compute.validation.osap_validation`) rather than on `compute.orchestrator.osap` itself — verified this is the correct target via a standalone Python sanity check (a deferred `from X import Y` re-reads the current `X.Y` attribute at call time, so patching the source module is picked up). No network, no real OSAP/qlib library. Test count: headline = 17, Files-list = 17 (both equal `grep -c "^def test_"` on the file — the 21 pytest-collected-ID figure above is reported for completeness per the parametrize expansion, matching how R4's own 23-vs-25 `def`-vs-collected gap was handled).

**Files**:
- `compute/orchestrator/osap.py` (new)
- `compute/main.py` (OSAP block replaced with an 18-line call + unpack, one import added: `from compute.orchestrator.osap import run_osap_pipeline`)
- `tests/test_orchestrator/test_osap.py` (new — 17 tests, 21 collected cases)
- `PHASE_STATUS_INFLIGHT.md` (this entry)

**Verification ladder** (all green): `ruff check compute/ tests/` PASS · `python -m compute.output.schema_check` in-sync (no drift) · `python -m pytest tests/test_orchestrator/ -m "not network" -q` → 109 passed (88 pre-existing R1-R5 + 21 new) · `python -c "import compute.main"` clean (no dangling refs) · full offline suite `python -m pytest tests/ -m "not network" -q` → 3360 passed, 10 skipped, 1 failed (`tests/test_features/test_alpha158_replicate.py::test_C1_accounting_equation_holds_for_all_inputs`, a pre-existing `hypothesis.core.DeadlineExceeded` flake in the UNTOUCHED Alpha158 block — reproduced identically with R6's changes `git stash`ed on top of clean `main`, confirming it is unrelated to this refactor) · targeted re-run of `tests/test_output/test_wall_clock_schema.py` (the one file whose harness drives `run_weekly_compute` through the OSAP block) → all 5 tests PASSED including `test_osap_wall_clock_none_on_pipeline_failure`, no patch-target update needed.

**R7 follow**: output accumulators (R7) — the final slice. Remains byte-identical.

---

## Phase 9.3 — Broad Investable US RANKED path, DISPATCH-ONLY (in flight, 2026-07-01)

**Branch**: `claude/phase-9-3-broad-universe-ranked`. Wires `QR_UNIVERSE=broad_investable_us` as an actual ranked/scored universe, gated behind a manual `workflow_dispatch` value — the scheduled weekday cron default stays `sp1500`, UNCHANGED. This is the RANKING wiring only; the AI-pick basket + backtest stay SP500-only (deferred to a future Phase 9.4 PR), and the frontend disclaimer for a broad-universe run is also Phase 9.4 scope.

**Scope**: `compute/main.py`'s universe-selector seam (the `if sp900 / elif sp1500 / else sp500` chain, ~line 1160) gains a fourth branch: `elif config.QR_UNIVERSE == "broad_investable_us":`. It loads the ~6,883-name Broad Investable US candidate pool via `candidates_to_universe_frame(fetch_broad_universe_candidates())` — both from `compute/ingest/broad_universe.py`, `candidates_to_universe_frame` being NEW this PR — and makes it the `universe` frame fed to Step 1. Step 1 (`fetch_all_prices`) therefore fetches prices for the FULL candidate pool on this path, not a restricted sp1500 dict.

**Screen-then-reduce sequencing**: immediately after Step 1 prices land (and after the existing, unconditional Phase-9.1 probe block runs — which now ALSO benefits on this path: its `broad_universe_screened_count` / `_coverage_pct` describe the TRUE screen result rather than the sp1500-restricted lower bound the probe-only slice produced), a new gated block (`if config.QR_UNIVERSE == "broad_investable_us":`) calls `select_broad_universe_survivors(candidates=universe, prices_by_ticker=prices_by_ticker)` — a NEW function in `broad_universe.py`, deliberately distinct from the Rule-18 diagnostic `screen_broad_universe_investability` (which stays untouched and keeps returning COUNTS for the probe's Metadata fields; the new function returns a `set[str]` of surviving tickers). `df` / `rows` / `prices_by_ticker` / `adv_by_ticker` are all reduced to the ~3,545 survivors BEFORE Step 2 (fundamentals) — so EDGAR load (fundamentals, the tier2 8-K loop, the Form-4 loop, every per-ticker scoring step) runs ONLY on survivors, never on all ~6,883 candidates. This bounds runtime to roughly the same order of magnitude as the existing sp1500 path; no `EDGAR_MAX_WORKERS` / tenacity-policy change is needed for this scale (perf-engineer confirmed via the existing #668 timeout budget).

**P1-G3 methodology gate** (methodology-scientist ratified pre-registration): non-survivors are REMOVED from the peer set entirely — they are NOT emitted as a `low_liquidity` annotate. That annotate exists for names that ARE ranked but trade thinly (S&P 1500 Slice 4, #527); a name that never enters the scored frame on the broad-universe path has nothing to annotate. This is a deliberate partition from the sp1500 path's liquidity-handling.

**Failure mode is ABORT, not fall-through** (deliberate design choice, revised mid-build): unlike most graceful-degradation blocks in `run_weekly_compute` (which fall through to a smaller/None result and keep going), a raised exception from `select_broad_universe_survivors` returns `0` — mirroring the `assert_sec_api_usable` / `MIN_VALID_TICKERS` abort idiom already used elsewhere in this function — rather than falling through to scoring the FULL unscreened ~6,883-name candidate frame. Silently scoring the unscreened frame would double the fundamentals-fetch volume this PR explicitly bounds (a stated task requirement: "control runtime") and risk tripping the CI timeout budget; on this dispatch-only path, abort-and-preserve-last-good-data is the safer failure mode than an unbounded-runtime partial run. The `MIN_VALID_TICKERS` post-screen abort gate (mirrors the Step-1 gate) is unchanged / additive on top of this.

**Metadata.universe label**: the existing 3-way ternary (`"SP1500"` / `"SP900"` / `config.UNIVERSE`) gains a fourth case: `"BROAD_INVESTABLE_US"` when `config.QR_UNIVERSE == "broad_investable_us"`. Restructured from a flat ternary into a nested if/else for readability at 4 cases — semantically identical for the 3 pre-existing cases (locked by a source-inspection regression test).

**`derive_index_memberships` extension** (`compute/ingest/universe.py`): the internal `_SP600_COHORTS = {"sp600"}` russell1000-proxy-suppression guard is renamed `_NO_RUSSELL1000_PROXY_COHORTS = {"sp600", "broad"}`. A `cohort="broad"` ticker (set unconditionally by `candidates_to_universe_frame`) never gets the `russell1000` proxy tag even with a known positive market cap — unlike the S&P 900 (whose floor structurally sits above the Russell 1000 cutoff), the broad, unfiltered-at-the-top candidate pool includes genuine micro-/small-caps well below that cutoff, so a positive market cap alone does not imply Russell 1000 membership there. Pure rename + set-extension — zero behaviour change for any pre-existing cohort value (`sp500`/`sp400`/`sp600` all evaluate the membership test identically to before; regression-locked by 3 dedicated tests).

**P1-G4 re-normalization disclosure** (methodology-REQUIRED, documented not code-enforced in this PR): broadening the scored universe from S&P 1500 (~1504 names) to Broad Investable US (~3,545 names) RE-BASES every cross-sectional percentile and sector median the 8-pillar composite uses — a score computed on this path is NOT comparable to the same ticker's sp1500-cron score. The caveat is documented in the `broad_universe.py` module docstring, the `main.py` universe-selector seam comment block, and CLAUDE.md §Gotchas. No frontend-facing disclaimer ships in this PR (Phase 9.4 scope).

**Sector/CIK shaping** (`candidates_to_universe_frame`, new function): GICS sector is not available from the SEC company-tickers source, so every candidate row gets `sector="Unknown"` — this mirrors the EXACT precedent already in `compute/ingest/universe.py`'s S&P 400/600 Wikipedia-table loader (which degrades to `sector="Unknown"` when its source table lacks a sector column). Downstream tolerance was verified, not assumed: sector-exclusion checks in `compute/valuation/applicability.py` are exact-string comparisons against `"Financials"`/`"Utilities"` (never trip on `"Unknown"`), `get_cost_of_equity` falls back to the flat 10% default for any unmapped sector, and the peer-grouping walk (`_build_peer_groupings` in `main.py`) buckets every `"Unknown"`-sector name into one large peer group (comfortably clears `MULTIPLES_MIN_PEERS=8`). `cik` is zero-padded `cik_str` when present, else `None` — a missing CIK falls back to `fetch_fundamentals`'s existing `Company(cik or ticker)` ticker-symbol resolution (same fallback the S&P 400/600 loader already relies on), at the cost of bypassing the snapshot parquet cache for that ticker.

**PANDAS 3.0 BUG CAUGHT AND FIXED mid-build** (this repo's `uv.lock` pins pandas 3.0.3, not the stale 2.3.3 in an out-of-sync local `.venv` — verified against a fresh `uv sync`): the first implementation of `candidates_to_universe_frame`'s `cik` column construction assigned a Python list (then `pd.array(..., dtype=object)`) containing a mix of strings and `None` directly into a DataFrame column. Under pandas 3.0's automatic string-dtype inference, this silently coerced every `None` back into a float `NaN` — even with `dtype=object` explicitly requested on the source array. This is a real corruption risk, not just a test nuisance: `bool(float("nan"))` is `True`, so the downstream `r.get("cik") or ""` pattern (used throughout `main.py` / `_fundamentals_one`) would keep the NaN rather than falling through to the empty-string default, and `str(nan_value)` would hand `fetch_fundamentals` the literal string `"nan"` as a CIK — a malformed EDGAR lookup that would have silently corrupted fundamentals for every broad-universe candidate with a missing/NaN `cik_str`. Fix: wrap the values in `pd.Series(_cik_values, dtype=object, index=out.index)` BEFORE assigning to the column — constructing the Series with an explicit dtype up front bypasses the content-based string-dtype inference that a subsequent bare-list/array assignment triggers. Verified with a minimal repro against the exact pinned pandas version; locked by `test_cik_none_when_cik_str_value_missing` (caught the bug on first test run) + 3 sibling CIK-handling tests.

**No schema change**: the existing 6 `broad_universe_*` Metadata fields (Phase 9.1) suffice — reused rather than duplicated. Distinguish "probe-only, still an sp1500 cron" (lower-bound `broad_universe_screened_count` semantics, unchanged) from "the ranked broad-universe run" (true-count semantics) via `Metadata.universe == "BROAD_INVESTABLE_US"`. `schema_check` confirmed clean (no drift — only comments changed in `schemas.py`).

**DEFAULT-PATH-UNCHANGED proof**: `git diff --stat -- compute/main.py` shows 146 insertions / exactly 2 deletions (one stale comment line, one ternary line expanded into a semantically-identical nested if/else) — no existing sp500/sp900/sp1500 branch body was touched. Locked by 6 `TestDefaultPathUnchangedRegressionGuard` tests in the new test file that pin EXACT verbatim source snippets (not just behavioural assertions) for the sp900 branch, the sp1500 branch's ranked-universe assignment line, and the default sp500 `else` branch body, plus a structural test confirming the new `elif` sits strictly between the sp1500 `elif` and the final `else`.

**Files**:
- `compute/ingest/broad_universe.py` (module docstring extended; two new functions: `candidates_to_universe_frame`, `select_broad_universe_survivors`)
- `compute/ingest/universe.py` (`derive_index_memberships`: `_SP600_COHORTS` → `_NO_RUSSELL1000_PROXY_COHORTS = {"sp600", "broad"}`, docstring updated)
- `compute/main.py` (new `elif` branch in the universe-selector seam; new survivor-reduction block between the Phase-9.1 probe and Step 2 fundamentals; `Metadata.universe` ternary extended to 4 cases)
- `compute/config.py` (`QR_UNIVERSE` docstring — documents the new accepted value + fixes a stale "cron sets sp900" comment left over from the Slice 7 flip)
- `compute/output/schemas.py` (comments only — updates the `broad_universe_*` field docstrings to describe the dual probe-vs-ranked semantics; NO field/type change)
- `tests/test_ingest/test_broad_universe_ranked.py` (new — 40 offline tests: `candidates_to_universe_frame` shaping/CIK-safety, `select_broad_universe_survivors` screen behaviour incl. a look-ahead-guard analog, `derive_index_memberships` broad-cohort + regression guards, `run_weekly_compute` wiring via source inspection, the 6 default-path-unchanged regression tests, the abort-vs-fall-through design lock, config docstring sanity)
- `PHASE_STATUS_INFLIGHT.md` (this entry)
- `CLAUDE.md` §Gotchas (new entry documenting the ranked path, alongside the existing Phase-9.1 probe entry which was lightly amended to cross-reference it)

**Test hygiene observation (out of scope, flagged for test-engineer / ci-triage-engineer)**: a full `pytest -m "not network"` run in this sandbox left `data/warehouse/_manifest.parquet` + 3 sibling warehouse parquet files modified on disk even though every `test_warehouse/*` file appears well-isolated (48 `monkeypatch`/`tmp_path` usages in `test_warehouse.py` alone). Reproduced twice, reverted both times (`git checkout -- data/warehouse/`), zero relationship to this PR's 5 touched files — likely some module-level `WAREHOUSE_DIR` default a specific test forgot to monkeypatch. Not investigated further (out of `compute-builder` scope for this task). Separately, `tests/test_features/test_alpha158_replicate.py::test_C1_accounting_equation_holds_for_all_inputs` (a Hypothesis property test, unrelated module) fails with `hypothesis.errors.DeadlineExceeded` on a completely unmodified `origin/main` checkout at the same commit this branch is built on (verified via an isolated detached-HEAD worktree) — a pre-existing sandbox-CPU-speed flake, not a regression from this PR.
...
## PR #259-R7b — orchestrator refactor R7b (per-ticker Step-8 loop) — FINAL slice (in flight, 2026-07-02)

**Branch**: `claude/orchestrator-refactor-r7b`. The FINAL slice of the 7-PR incremental refactor of `run_weekly_compute` (issue #259) — the largest and highest-risk slice, on the real scoring/valuation hot path (unlike the observability-only R6 OSAP block). Second sub-slice after R7a (pre-loop membership maps, #678). With this merged, `run_weekly_compute` is fully decomposed into the `compute/orchestrator/` package and #259 is complete.

**Scope (PURE VERBATIM CODE MOVE — byte-identical output)**: extracts the entire Step-8 combined per-ticker loop (~760 lines: 6-method fair-price ensemble + price-history write + the ~20 dedup-then-append `valuation_warnings.append` sites + StockSummary/StockDetail construction & per-stock JSON write) PLUS its ~25 pre-loop accumulator/counter inits out of `run_weekly_compute` into a new function `run_per_ticker_loop(...)` in `compute/orchestrator/per_ticker.py` (the module R7a created), returning a `PerTickerLoopResult` dataclass. No logic change, no statement reordering, same effects/log-lines/exception-handling.

**Smells #5 / #8 DEFERRED (not consolidated)**: the per-ticker loop's spaghetti smells #5 (the ~20× dedup-then-append `if X not in valuation_warnings: valuation_warnings.append(X)` pattern → a `WarningEmitter`) and #8 (`check_rim_applicability` called twice per ticker — flat-Ke + sector-Ke) were LEFT VERBATIM. Folding them in would change append/call semantics and jeopardise the byte-identical proof; they are routed to a small follow-up PR (a `WarningEmitter` refactor that is itself byte-identical-provable in isolation). This keeps R7b a trivially-verifiable move — the whole point of the #259 incremental discipline.

**Input/output contract** (enumerated mechanically, grep-per-name):
- INPUTS (keyword params): `df` · `snapshots` · `pillar_df` · `risk_flags` · `beneish_results` · `dechow_results` · `rem_results` · `post_split_results` · `tier2_results` · `histories` · `historical_metrics` · `universe_metrics` · `by_sector` · `by_sub_industry` · `broad_ex_fin_util` · `adv_by_ticker` · `prices_by_ticker` · `imputed_by_ticker` · `sector_pillar_baselines` · `form4_diagnostics` · `osap_signal_map` · `composite_osap_adjusted` · `cohort_by_ticker` · `memberships_by_ticker` · `multi_class_flagged_tickers` · `entered` · `exited` · `asof_date` · `now` · `filing_lag_fn` (=`_filing_lag`) · `build_raw_metrics_fn` (=`_build_raw_metrics`). All assigned before the loop in `run_weekly_compute`; the two `_fn` params pass the existing module-private helpers so they are not moved.
- OUTPUTS (`PerTickerLoopResult` fields, all read AFTER the loop — feed the `Metadata(...)` constructor + Step-13.5 warehouse): `cross_source_wall_clock_seconds` · `summaries` · `all_details` · `loss_avoidance_size_invariant_firing_count` · `share_count_extraction_missing_count` · `fundamentals_unavailable_count` · `low_liquidity_annotate_count` · `extreme_estimate_majority_count` · `extreme_estimate_majority_lowapp_count` · `cross_source_disagreement_count` · `cross_source_delta_histogram` · `cross_source_delta_by_ticker` · `_cs_yf_market_cap_by_ticker` · `_cs_yf_shares_by_ticker` · `_median_trimmed_by_ticker` · `exchange_by_ticker` · `country_by_ticker` · `_dividend_yield_pct_by_ticker` · `_security_type_by_ticker` · `value_trap_risk_count_without_sector_coe` (+ `_with_` + `_two_factor_shadow_count` + both `_by_sector` dicts) · `insider_sell_cluster_firing_count` · `c_suite_unusual_sell_firing_count` · `form4_rule10b5_one_excluded_count` · `multi_class_aggregate_shares_suspected_count`.
- LOOP-INTERNAL-ONLY (correctly NOT returned): `detail_count` / `history_count` / `fair_price_count` (consumed only by the loop's own end-of-loop summary log line, which moved into the function) + `_pays_dividend_by_ticker` / `_payout_ratio_by_ticker` (written into each `StockDetail` inside the loop; the coverage aggregation post-loop reads only `_dividend_yield_pct_by_ticker`, which IS returned).

**`compute/main.py` changes**: added `run_per_ticker_loop` to the orchestrator import; removed the ~760-line loop + ~25 init lines; replaced with the retained R7a `build_ticker_membership_maps` unpack → a `result = run_per_ticker_loop(...)` call → a rebind block aliasing `result.<field>` into the same local names the downstream Metadata/warehouse code already uses (lowest-diff-downstream — the `Metadata(...)` constructor + Step-9 sanity + Step-13.5 warehouse code are UNCHANGED). Imports used only by the loop (e.g. `fetch_yfinance_exchange`, `write_stock_detail`/`write_stock_history`, ensemble/valuation-warning helpers) moved into the new module; imports still used elsewhere in main.py left in place. Net: main.py ~-1056 lines, per_ticker.py +~1415.

**Test patch-target fix** (mechanical, caused by the move): 4 tests in `tests/test_main.py` (`test_step8_exchange_mapping_known_code` / `_none_code` / `_non_us_code_passthrough` / `_coverage_excludes_none_entries`) did `monkeypatch.setattr("compute.main.fetch_yfinance_exchange", …)`; since that call site moved, the symbol is no longer a `compute.main` attribute → `AttributeError`. Updated the 4 patch targets to `"compute.orchestrator.per_ticker.fetch_yfinance_exchange"` (the new home — confirmed imported there). The tests exercise `exchange_name`/`country_for_exchange` directly (imported from `compute.ingest.cross_source`, unaffected); the patch was vestigial, only the target string needed the move. Caught by the full offline suite (4 failed pre-fix → 0 after).

**Schema triple**: untouched — no `schemas.py`/`types.ts`/`schema-snapshot.json` change; `schema_check` in-sync. `compute/scoring/**`, `compute/valuation/**`, `compute/ingest/**`: NOT modified. Defense layer UNCHANGED at 38.

**Tests**: `tests/test_orchestrator/test_per_ticker.py` extended with offline `run_per_ticker_loop` coverage (result-dataclass shape + accumulator values on a synthetic mini-universe). No network.

**Files**:
- `compute/orchestrator/per_ticker.py` (extended — `run_per_ticker_loop` + `PerTickerLoopResult`)
- `compute/main.py` (loop + inits replaced with the call + rebind; imports moved)
- `tests/test_orchestrator/test_per_ticker.py` (extended)
- `tests/test_main.py` (4 patch-target updates)
- `PHASE_STATUS_INFLIGHT.md` (this entry)

**Verification ladder** (all green): `ruff check compute/ tests/` PASS · `python -m compute.output.schema_check` in-sync · `python -m pytest tests/test_orchestrator/ -m "not network" -q` → 127 passed · `python -c "import compute.main"` clean · full offline suite `python -m pytest tests/ -m "not network"` → 3417 passed / 10 skipped (env-gap skips only) with the 4 exchange-mapping tests failing ONLY on the pre-fix patch target → after the fix `python -m pytest tests/test_main.py` → 61 passed (the definitive end-to-end byte-identical confirmation: the `run_weekly_compute` harness drives the full loop through the new function). The pre-existing `test_alpha158_replicate.py::test_C1` hypothesis DeadlineExceeded sandbox flake (unrelated, untouched Alpha158 block) is the only non-env failure class and does not implicate this move.

**#259 COMPLETE with this merge**: R1 (ComputeState facade) · R2 (prices) · R3 (fundamentals) · R4 (Form-4 + smell #9) · R5 (Tier-2) · R6 (OSAP) · R7a (pre-loop maps) · R7b (per-ticker loop). Follow-up: the deferred smell #5 (WarningEmitter) / #8 (rim double-call) consolidation as a small standalone byte-identical PR.
...
## PR (branch `claude/broad-investable-us-6jw9gv`) — production-safety commit-guard for the `broad_investable_us` dispatch (in flight, 2026-07-02)

**Iteration ① of the broad-universe floor-scoping loop** — the PR-2 prerequisite anticipated by #684's methodology-scientist ratification ("the actual dollar re-pin is PR-2's empirical-sweep job"). A CI-workflow-only guard, no schema/scoring change, that makes a `QR_UNIVERSE=broad_investable_us` dispatch of `compute-rankings.yml` SAFE to actually run and collect its sweep data — until this PR, that dispatch option existed in the workflow but firing it would have been a production incident.

**The problem this closes**: `compute-rankings.yml`'s "Commit JSON outputs" step (`git add frontend/public/data/` + `git add data/warehouse/` + `git commit` + `git pull --rebase && git push`) was UNCONDITIONAL on `QR_UNIVERSE`, and `compute/main.py` writes `rankings.json`/`metadata.json`/`stocks/*.json` to `config.DATA_DIR` (= `frontend/public/data/`) with no universe-conditional output path. A `broad_investable_us` dispatch (the Phase 9.3 RANKED path, #676) would therefore have committed ~3,545-name broad-universe rankings — re-based cross-sectional percentiles, `sector="Unknown"`, flat-10%-CoE fallback — straight to `main`. Vercel rebuilds from `main`, so the LIVE public site would have shown broad-universe data instead of the S&P 1500, and `prune_orphan_stock_files()` would have deleted every sp1500 per-stock JSON (only a partial ticker overlap between the two universes). No `broad_investable_us` dispatch has run against `compute-rankings.yml` to date — the risk was armed but never yet triggered.

**The fix** — two complementary `if:` guards in `compute-rankings.yml`, both keyed off the SAME `env.QR_UNIVERSE` (the workflow's existing single resolved value, `${{ github.event.inputs.universe || 'sp1500' }}`):
- `Commit JSON outputs` step gains `if: ${{ env.QR_UNIVERSE != 'broad_investable_us' }}` — the ENTIRE step (both `git add` targets, the commit, and the rebase-push loop) is skipped on the broad path. Every other value — sp500 / sp900 / sp1500, AND the scheduled cron (where `github.event.inputs` is null and `QR_UNIVERSE` resolves to `sp1500` via the existing `||` fallback) — takes this branch and commits EXACTLY as before. Pure add-on; byte-identical for every non-broad path.
- New `Upload broad-universe sweep output (artifact, NOT committed)` step, `if: ${{ env.QR_UNIVERSE == 'broad_investable_us' }}`, `actions/upload-artifact@043fb46d1a93c77aae656e7c1c64a875d1fc6a0a # v7` (the SHA already pinned for this action in `precache-broad-universe.yml`), uploads `frontend/public/data/metadata.json` (carries the 6 `broad_universe_survivors_at_*`/`adr_suspect`/`coverage`/`sector_unknown` sweep fields from #684) + `frontend/public/data/rankings.json` (needed for the eventual C5 rank-stability comparison) as artifact `broad-universe-sweep-output`, `if-no-files-found: error` (fail loud rather than silently upload nothing), `retention-days: 30` (mirrors the `backfill-warehouse.yml` precedent for "generated-but-never-committed" artifacts).

**Placement choice (deliberate)**: the upload step is placed immediately after "Run weekly compute" — BEFORE the optional "Refresh portfolio backtest (PIT)" step (up to 55 more minutes, `continue-on-error: true`) — rather than at the end of the job next to its complementary commit-step guard. The broad-universe warm runtime is not yet empirically confirmed (the job's own timeout-minutes history projects compute alone at ~280-320 min warm-worst-case against the 360-min GitHub Actions hard per-job cap established by the gate-C1 fix, #681), so capturing the artifact as early as possible avoids losing the sweep data to the same "job cancelled, nothing salvaged" failure class gate-C1 hit — an artifact already uploaded by a completed step survives a later step's timeout/cancellation. The backtest-refresh step itself is left completely untouched (its SP500-only output was never going to be committed on the broad path anyway once the commit-step guard lands, so gating it too was unnecessary given the upload already happens before it runs).

**Verified no other path can commit broad-universe data to `main`**: grepped every workflow + `compute/main.py` for `git push`/`git commit`/`git add`/`DATA_DIR`/`WAREHOUSE_DIR`. `precache-edgar.yml` explicitly excludes `broad_investable_us` from its `universe` dispatch options (gate-C1 fix, #681) and never commits. `precache-broad-universe.yml` has no `universe` input at all (implicitly always-broad) and never commits (cache-warming only). `pre-merge-prod-sim.yml` hardcodes `QR_UNIVERSE: sp500` and never commits. `backfill-portfolio.yml` / `backfill-warehouse.yml` don't read `QR_UNIVERSE` at all (their own hardcoded SP500/PIT-ledger scope) and both already carry `if: github.ref_name != 'main'`-class guards against a direct-to-main dispatch. Every `frontend/public/data/*` writer in `compute/` (`writer.py`, `portfolio_writer.py`, `ic_decay.py`, `ranking_history.py`, `basket_rule_validation.py`) funnels through the single `config.DATA_DIR` tree the one gated `git add` line already covers — no per-writer changes were needed.

**Constraints held**: no schema change, no scoring change, CI-workflow-only. sp1500 / sp900 / sp500 dispatches + the scheduled cron are byte-identical (confirmed via `yaml.safe_load` step enumeration — the two new `if:` conditions land on exactly the two intended steps, no other step in the 13-step `compute` job gained a condition). `compute/main.py` was NOT touched — the guard lives entirely at the workflow level, so the runner's discarded checkout is the only place broad data can ever land, per the task's stated preference.

**Unblocks**: a `workflow_dispatch` of `compute-rankings.yml` with `universe: broad_investable_us` can now safely run end-to-end and produce the `broad-universe-sweep-output` artifact (the PR-2 empirical floor-sweep numbers — survivor counts at $5M/$10M/$15M ADV floors, etc.) without any risk of touching the production site or `main`'s git history. That dispatch run itself, and the resulting floor re-pin decision, remain future work.

**Files**: `.github/workflows/compute-rankings.yml` (two `if:` guards + one new step), `PHASE_STATUS_INFLIGHT.md` (this entry).

**Verification**: `python tools/preflight.py` → GREEN (ruff / doc-count / model-pin / agent-hook / defense-layer-count rungs PASS; schema_check / pytest / tsc correctly SKIP — no compute/tests/schema/frontend surface touched). `python -c "import yaml,sys; yaml.safe_load(open('.github/workflows/compute-rankings.yml'))"` → parses clean. Manual re-read of the 13-step `compute` job (enumerated via `yaml.safe_load`) confirms only steps 8 (`Upload broad-universe sweep output`) and 12 (`Commit JSON outputs`) carry the new `if:` conditions; all 11 other steps are untouched.

**Gate**: DRAFT — no live `broad_investable_us` dispatch has exercised this diff yet (by construction; that dispatch was unsafe until this PR). Will get a `security-reviewer` pass (workflow-permissions / `if:`-logic review) before Mark-Ready per the task brief.

---

## PR #695 (branch `claude/ai-picks-calculation-review-zwyb0x`) — AI-pick backtest methodology corrections: T+1-close execution + benchmark-basis guard + unrounded headline + survivorship disclosure (in flight, 2026-07-03)

**Origin**: user-requested methodology audit of the AI-pick calculation vs international performance-measurement standards ("หลักการคิดคำนวณของ ai picks มีอะไรที่ไม่ถูกต้องหรือไม่ ตามหลักสากล"). Audit verdict: core structure sound (total-return vs total-return on Adj Close both sides · PIT fundamentals + 45-day filing lag · survivorship-corrected membership ledger · net-of-cost headline · correctly-labeled Modified-Dietz MWR) with 5 genuine deviations, all fixed here.

**F1 — T+1-close execution convention (track-record restatement, methodology-RATIFIED)**: fills previously landed at the close of the first trading day ON-OR-AFTER the decision date T (`bisect_left`); when T is a trading day the decision close IS the fill close — a zero-implementation-lag fill no follower can obtain (the cron publishes rankings ~22:00 UTC, after the US close). methodology-scientist verdict RATIFY-WITH-CONDITIONS (Fama-French event-time formation · Jegadeesh-Titman 1994 skip-period · Lo-MacKinlay 1990 non-synchronous trading · Novy-Marx-Velikov 2016; the flip is justified EX ANTE by publish mechanics, not outcome comparison — DSR `n_trials` stays 16): fills move to the first trading day STRICTLY AFTER T (T+1 close, `bisect_right`) at genuine TRADE-FILL sites only — `_snap_to_trading_day` in the backfill + the DSR mirror in `basket_rule_validation.py`, `position_returns` entry legs (`_close_on_or_after` → `_close_strictly_after`) and non-latest-quarter terminals via the new `_terminal_close` (symmetric sell-fill, close-on-or-before fallback for delisting per Shumway 1997 — keeps the Carino/TWR reconciliation counters at machine epsilon); measurement MARKS (`_snap_monthly_nav` month-end resample, quarterly boundary marks) stay on-or-before by design. Additive artifact fields `rebalances[].execution_date` + `meta.execution_convention="t_plus_1_close"`(+`_note`) — free-form `backtest_pit.json`, NOT the Pydantic↔TS↔snapshot triple (C-2/E artifact-vs-triple precedent). Selection thresholds (composite 65 / floor 5 / hold-band 55) intentionally NOT re-swept under the new timing (disclosed in `_DISCLAIMER_STATIC`; keeps the restated NAV a fixed-parameter check). Frontend reads `execution_date ?? date` for per-holding entry prices — byte-identical on the committed old-timing artifact (backward compatible; `next build` verified against it). The NAV track record restates only after the post-merge `backfill-portfolio.yml` dispatch regenerates the artifact.

**F2 — benchmark price-basis guard**: `write_benchmarks_json` silently fell back "Adj Close" → "Close" (a future yfinance format change would flip the comparison to a price-return benchmark unnoticed). Now warns per symbol + emits `payload["price_basis"]`; backfill propagates `meta.benchmark_price_basis` + a dynamic disclaimer clause when any benchmark sits on raw-close basis. **F3 — unrounded headline outperformance (frontend)**: `buildView` derived period returns from `Math.round`-ed dollar chart points before subtracting; now derives from unrounded NAV ratios (last-finite/anchor, mirroring `periodGross`) and rounds exactly once at display; 5 new vitest tests incl. a near-zero-spread regression the rounded path collapses to 0.0%. **F4 — survivorship-drop disclosure**: removed-membership tickers dropped for unresolvable CIK / missing prices now listed in `meta.scoring_universe_removed_unavailable_tickers` + disclaimer clause when count > 0 (count field already existed). **F5 — docstring truthfulness**: "trade the next open" claims corrected to the actual T+1-close semantics everywhere.

NO schema bump (all new fields are free-form backtest artifact fields); rankings/scores/flags BYTE-IDENTICAL (backtest-only surface); defense layer UNCHANGED at 38. Ground-truth verification: an INDEPENDENT trade simulator (separate from `build_portfolio_nav`, fresh yfinance Adj Close) replays the published rebalances under both conventions — results posted on the PR. Full home-page numeric audit (rendered vs artifact-recomputed) run alongside.

**Files**: `scripts/backfill_portfolio_pit.py` · `compute/portfolio/position_returns.py` · `compute/validation/basket_rule_validation.py` · `compute/output/writer.py` · `scripts/experiment_rebalance_frequency.py` · `frontend/components/AiPickPortfolio.tsx` (+ new `.test.ts`) · `frontend/lib/data.ts` · `frontend/lib/types.ts` (artifact view-model only) · `tests/test_portfolio/test_position_returns.py` + new deterministic pin tests (test-engineer pass, ratification condition 6) · `PHASE_STATUS_INFLIGHT.md` (this entry).

**Verification**: `ruff check .` PASS · offline pytest 3494 P / 10 skipped (pre-existing optional-dep gates) · portfolio/output/validation suites isolated re-run 654 P · `tsc --noEmit` PASS · `next build` PASS against the OLD committed artifact (backward compat) · vitest 334/334. Gate: DRAFT — quantrank-reviewer + phase-coordinator Mode B before Mark-Ready; post-merge `backfill-portfolio.yml` dispatch restates the artifact.
