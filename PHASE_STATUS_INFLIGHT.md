...
- `compute/main.py` accumulates `mad_scalefree` per ticker inside the existing per-ticker loop (the
  prices DataFrame is already in hand — zero added fetches, ~2 `.tail().mean()` per ticker) plus the
