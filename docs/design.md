...
**Related design docs** — three files with distinct roles; keep brand values in sync
across them to avoid drift (the cause of the 2026-06-03 `#047857` vs `#15803D` mislabel):

- **this file** (`docs/design.md`) — the **canonical** project visual spec (colors,
  type, components). CLAUDE.md points here.
- [`DESIGN.md`](../DESIGN.md) (repo root) — the `impeccable` design-skill doc (a
  machine-read subset; regenerated by `$impeccable init` / `document`).
- [`ledgercraft-DESIGN.md`](../ledgercraft-DESIGN.md) (repo root) — the upstream
  **LedgerCraft source spec** QuantRank adopted (provenance only; it uses forest green
  `#15803D` = green-700, which the QuantRank build ships as the cooler Tailwind
  emerald-700 `#047857` — see the Hex note under §Color).
...
| **Neutral** (slate) | `#94A3B8` | `slate-400` | Borders, disabled states, placeholders |
